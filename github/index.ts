...
import { createSealcodeClient } from "@sealcode-ai/sdk"
...
const { client, server } = createSealcode()
...
try {
  ...
  console.log("sealcode session", session.id)
  if (shareId) {
...
function createSealcode() {
  ...
  const proc = spawn(`sealcode`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createSealcodeClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/sealcode|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/sealcode` or `/oc`")
  }
}
...
async function assertSealcodeConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to sealcode server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.github.com/mnmalali/sealcode" : "https://github.com/mnmalali/sealcode"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.github.com/mnmalali/sealcode/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("sealcode-github-action")
    response = await fetch("https://api.github.com/mnmalali/sealcode/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/sealcode" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/sealcode") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to sealcode...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "sealcode-agent[bot]"`
  await $`git config --global user.email "sealcode-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `sealcode/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/sealcode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[sealcode session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
