import type {
  ...
  Session,
  Todo,
} from "@sealcode-ai/sdk/v2/client"
...
export const loadGlobalConfigQuery = (sdk: SealcodeClient) =>
  queryOptions({
    queryKey: ["config"],
    queryFn: () => retry(() => sdk.global.config.get().then((x) => x.data!)),
  })

export const loadProjectsQuery = (sdk: SealcodeClient) =>
  queryOptions({
    queryKey: ["project"],
    queryFn: () =>
      retry(() =>
        sdk.project.list().then((x) => {
          return (x.data ?? [])
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("sealcode-test"))
            .slice()
...
export const loadProvidersQuery = (directory: string | null, sdk: SealcodeClient) =>
  queryOptions({
    queryKey: [directory, "providers"],
    queryFn: () => retry(() => sdk.provider.list().then((x) => normalizeProviderList(x.data!))),
  })

export const loadAgentsQuery = (directory: string | null, sdk: SealcodeClient) =>
  queryOptions({
    queryKey: [directory, "agents"],
    queryFn: () => retry(() => sdk.app.agents().then((x) => normalizeAgentList(x.data))),
  })

export const loadPathQuery = (directory: string | null, sdk: SealcodeClient) =>
  queryOptions<Path>({
