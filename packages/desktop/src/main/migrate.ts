...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.sealcode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `sealcode.settings.dat` is special: it maps to the `sealcode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"sealcode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // sealcode.settings.dat → the electron settings store ("sealcode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "sealcode.global.dat").
  const storeName = filename === "sealcode.settings.dat" ? "sealcode.settings" : filename
