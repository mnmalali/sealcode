...
import "./styles.css"
import { useTheme } from "@sealcode-ai/ui/theme"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.SEALCODE_CHANNEL === "prod" &&
...
const deepLinkEvent = "sealcode:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__SEALCODE__ ??= {}
  const pending = window.__SEALCODE__.deepLinks ?? []
  window.__SEALCODE__.deepLinks = [...pending, ...urls]
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("sealcode.global.dat").getItem("language")
