...
Replace runtime Bun shell template-tag usage in `packages/sealcode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/sealcode/src` is removed except:
