...
Delete `packages/sealcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/sealcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/sealcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/sealcode`
- TUI plugin loader tests in `packages/sealcode` if runtime plugin API wiring changed
