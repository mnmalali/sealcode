...
- Author package entrypoint is `@sealcode-ai/plugin/tui`.
...
```json
{
  "$schema": "https://github.com/mnmalali/sealcode/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/sealcode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@sealcode-ai/plugin/tui`.
- `@sealcode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@sealcode-ai/plugin/tui"
...
- The value is a semver range checked against the running Sealcode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.sealcode` is absent, no check is performed (backward compatible).
...
- `sealcode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `sealcode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.sealcode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.sealcode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.sealcode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.sealcode` dir or `SEALCODE_CONFIG_DIR`, Sealcode installs `@sealcode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@sealcode-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Sealcode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@sealcode-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"sealcode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `sealcode.default`.
...
- Local plugins persist installed themes under the local `.sealcode/themes` area near the plugin config source.
...
- `--pure` / `SEALCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.sealcode/plugins/tui-smoke.tsx`
- Local vim plugin: `.sealcode/plugins/tui-vim.tsx`
- Local smoke config: `.sealcode/tui.json`
- Local smoke theme: `.sealcode/plugins/smoke-theme.json`
