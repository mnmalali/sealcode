...
Practical reference for a future `packages/server` split after the sealcode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/sealcode`.
...
Until enough shared service code lives outside `packages/sealcode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/sealcode`

It should not import `packages/sealcode` services while `packages/sealcode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on sealcode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/sealcode` implementation details.
