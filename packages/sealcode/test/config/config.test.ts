...
import { EffectFlock } from "@sealcode-ai/core/util/effect-flock"
...
import { CrossSpawnSpawner } from "@sealcode-ai/core/cross-spawn-spawner"
...
const managedConfigDir = process.env.SEALCODE_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "sealcode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "sealcode.json") {
...
test("creates global jsonc config with schema when no global configs exist", async () => {
  ...
  try {
    ...
    const content = await Filesystem.readText(path.join(tmp.path, "sealcode.jsonc"))
    expect(content).toContain('"$schema": "https://github.com/mnmalali/sealcode/config.json"')
  } finally {
    ;(Global.Path as { config: string }).config = prev
    await clear(true)
  }
...
test("does not create global config when SEALCODE_CONFIG_DIR is set", async () => {
  await using tmp = await tmpdir()
  await using custom = await tmpdir()
  const prevConfig = Global.Path.config
  const prevEnv = process.env.SEALCODE_CONFIG_DIR
  ;(Global.Path as { config: string }).config = tmp.path
  process.env.SEALCODE_CONFIG_DIR = custom.path
  await clear(true)
  ...
  try {
    ...
    expect(await Filesystem.exists(path.join(tmp.path, "sealcode.jsonc"))).toBe(false)
  } finally {
    ;(Global.Path as { config: string }).config = prevConfig
    if (prevEnv === undefined) delete process.env.SEALCODE_CONFIG_DIR
    else process.env.SEALCODE_CONFIG_DIR = prevEnv
    await clear(true)
  }
})
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "sealcode.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "sealcode.jsonc")
...
test("ignores legacy tui keys in sealcode config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://github.com/mnmalali/sealcode/config.json",
  ...
  try {
    ...
    await withTestInstance({
      ...
      fn: async (ctx) => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "sealcode.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["SEALCODE_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { sealcode: { options: { apiKey: "{env:SEALCODE_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["sealcode"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["SEALCODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["SEALCODE_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "sealcode.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("loads config from .sealcode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const sealcodeDir = path.join(dir, ".sealcode")
      await fs.mkdir(sealcodeDir, { recursive: true })
      const agentDir = path.join(sealcodeDir, "agent")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".sealcode", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .sealcode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const sealcodeDir = path.join(dir, ".sealcode")
      await fs.mkdir(sealcodeDir, { recursive: true })

      const agentsDir = path.join(sealcodeDir, "agents")
...
test("loads commands from .sealcode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const sealcodeDir = path.join(dir, ".sealcode")
      await fs.mkdir(sealcodeDir, { recursive: true })

      const commandDir = path.join(sealcodeDir, "command")
...
test("loads commands from .sealcode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const sealcodeDir = path.join(dir, ".sealcode")
      await fs.mkdir(sealcodeDir, { recursive: true })

      const commandsDir = path.join(sealcodeDir, "commands")
...
test("does not try to install dependencies in read-only SEALCODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.SEALCODE_CONFIG_DIR
  process.env.SEALCODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.SEALCODE_CONFIG_DIR
    else process.env.SEALCODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable SEALCODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "sealcode.json"),
        JSON.stringify({ $schema: "https://github.com/mnmalali/sealcode/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .sealcode config
      const projectDir = path.join(dir, "project")
      const sealcodeDir = path.join(projectDir, ".sealcode")
      await fs.mkdir(sealcodeDir, { recursive: true })
      ...
      // Local .sealcode config with different plugins
      await Filesystem.write(
        path.join(sealcodeDir, "sealcode.json"),
        JSON.stringify({
          $schema: "https://github.com/mnmalali/sealcode/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .sealcode config with some overlapping plugins
      await Filesystem.write(
        path.join(sealcodeDir, "sealcode.json"),
        JSON.stringify({
          $schema: "https://github.com/mnmalali/sealcode/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".sealcode")
...
// Note: preload.ts sets SEALCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .sealcode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .sealcode directory config enables it
      const sealcodeDir = path.join(dir, ".sealcode")
      await fs.mkdir(sealcodeDir, { recursive: true })
      await Filesystem.write(
        path.join(sealcodeDir, "sealcode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/sealcode")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/sealcode")
    ...
    if (urlStr.includes(".well-known/sealcode")) {
      ...
      return Promise.resolve(
        new Response(
          JSON.stringify({
            remote_config: {
              url: "https://config.example.com/sealcode.json",
              headers: {
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(wellknownFetchedUrl).toBe("https://example.com/.well-known/sealcode")
            expect(remoteFetchedUrl).toBe("https://config.example.com/sealcode.json")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "sealcode.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-sealcode@2.4.3", file)).toBe("oh-my-sealcode@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-sealcode@2.4.3", "file:///project/.sealcode/plugin/oh-my-sealcode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.sealcode/plugin/demo.ts", "file:///project/.sealcode/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.sealcode/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const sealcodeDir = path.join(projectDir, ".sealcode")
        const pluginDir = path.join(sealcodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("SEALCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["SEALCODE_DISABLE_PROJECT_CONFIG"]
    process.env["SEALCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["SEALCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["SEALCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .sealcode/ directories when flag is set", async () => {
    const originalEnv = process.env["SEALCODE_DISABLE_PROJECT_CONFIG"]
    process.env["SEALCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .sealcode directory with a command
          const sealcodeDir = path.join(dir, ".sealcode", "command")
          await fs.mkdir(sealcodeDir, { recursive: true })
          await Filesystem.write(path.join(sealcodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await withTestInstance({
        directory: tmp.path,
        fn: async (ctx) => {
          ...
          // Project .sealcode should NOT be in directories list
          const hasProjectSealcode = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectSealcode).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["SEALCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["SEALCODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["SEALCODE_CONFIG_DIR"]
      process.env["SEALCODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["SEALCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["SEALCODE_CONFIG_DIR"]
      } else {
        process.env["SEALCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("SEALCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["SEALCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["SEALCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["SEALCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["SEALCODE_CONFIG_DIR"] = configDirTmp.path

      await withTestInstance({
        directory: projectTmp.path,
        fn: async (ctx) => {
          const config = await load(ctx)
          // Should load from SEALCODE_CONFIG_DIR, not project
...
describe("SEALCODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in SEALCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["SEALCODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["SEALCODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://github.com/mnmalali/sealcode/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["SEALCODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["SEALCODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in SEALCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["SEALCODE_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.schema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "Sealcode Managed",
          PayloadIdentifier: "ai.sealcode.managed.test",
          PayloadType: "ai.sealcode.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://github.com/mnmalali/sealcode/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://github.com/mnmalali/sealcode/config.json")
})
