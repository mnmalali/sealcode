...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "sealcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "sealcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "sealcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "sealcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["sealcode.json", "sealcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "sealcode.json"),
        path.join(tmp.path, "sealcode.jsonc"),
        path.join(project, "sealcode.json"),
        path.join(project, "sealcode.jsonc"),
      ])
    })
  })
