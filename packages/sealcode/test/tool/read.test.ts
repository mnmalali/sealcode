...
describe("tool.read external_directory permission", () => {
  ...
  scout.live("does not ask for external_directory permission when reading configured references", () =>
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const cache = path.join(Global.Path.repos, "github.com", "sealcode-read-reference", "repo")
      ...
      const remoteDir = path.join(remoteRoot, "sealcode-read-reference")
      ...
      const dir = yield* tmpdirScoped({
        ...
        config: {
          reference: {
            docs: "sealcode-read-reference/repo",
          },
        },
      })
