...
const originalEnv = {
  SEALCODE_AUTH_CONTENT: process.env.SEALCODE_AUTH_CONTENT,
  SEALCODE_EXPERIMENTAL_WORKSPACES: process.env.SEALCODE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  Database.close()
  restoreEnv()
  process.env.SEALCODE_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@sealcode.test"`.cwd(dir).quiet()
...
describe("workspace CRUD", () => {
  ...
  test("create configures, persists, creates, starts local sync, and passes environment", async () => {
    await withInstance(async (instance) => {
      process.env.SEALCODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
      process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
      process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
      process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=sealcode-test"
      ...
      expect(JSON.parse(recorded.calls.create[0].env.SEALCODE_AUTH_CONTENT ?? "{}")).toEqual({
        test: { type: "api", key: "secret" },
      })
      expect(recorded.calls.create[0].env.SEALCODE_WORKSPACE_ID).toBe(workspaceID)
      expect(recorded.calls.create[0].env.SEALCODE_EXPERIMENTAL_WORKSPACES).toBe("true")
      ...
      expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=sealcode-test")
