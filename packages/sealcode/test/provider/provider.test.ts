...
async function markPluginDependenciesReady(dir: string) {
  ...
  await Bun.write(
    ...
    JSON.stringify({ packages: { "": { dependencies: { "@sealcode-ai/plugin": "0.0.0" } } } }),
  )
}

function paid(providers: Awaited<ReturnType<typeof list>>) {
  const item = providers[ProviderID.make("sealcode")]
...
test("ModelNotFoundError suggests catalog models for unloaded providers", async () => {
  ...
  await withTestInstance({
    directory: tmp.path,
    fn: async (ctx) => {
      remove(ctx, "SEALCODE_API_KEY")
      try {
        await getModel(ProviderID.sealcode, ModelID.make("claude-haiku-fake-model"), ctx)
        ...
        JSON.stringify({
          ...
          provider: {
            "cloudflare-ai-gateway": {
              options: {
                metadata: { invoked_by: "test", project: "sealcode" },
              },
            },
          },
        }),
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const configDir = path.join(dir, ".sealcode")
...
test("plugin config enabled and disabled providers are honored", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".sealcode", "plugin")
      await mkdir(root, { recursive: true })
      await Bun.write(
...
test("sealcode loader keeps paid models when config apiKey is present", async () => {
...
test("sealcode loader keeps paid models when auth exists", async () => {
  await using base = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "sealcode.json"),
