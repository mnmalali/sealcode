...
const directoryHeader = (dir: string) => HttpClientRequest.setHeader("x-sealcode-directory", dir)
...
describe("instance HttpApi", () => {
  ...
  it.live("emits a sync fence header for fixed-workspace mutations", () =>
    Effect.gen(function* () {
      const originalWorkspaceID = Flag.SEALCODE_WORKSPACE_ID
      Flag.SEALCODE_WORKSPACE_ID = WorkspaceID.ascending()
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          Flag.SEALCODE_WORKSPACE_ID = originalWorkspaceID
        }),
      )
        ...
        Effect.promise(() =>
          HttpApiApp.webHandler().handler(
            new Request(`http://localhost${path}`, {
              ...init,
              headers: { "x-sealcode-directory": dir, "content-type": "application/json", ...init?.headers },
            }),
            handlerContext,
          ),
        )
