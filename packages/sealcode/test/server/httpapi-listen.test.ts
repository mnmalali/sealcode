...
const original = {
  ...
  envPassword: process.env.SEALCODE_SERVER_PASSWORD,
  envUsername: process.env.SEALCODE_SERVER_USERNAME,
}
const auth = { username: "sealcode", password: "listen-secret" }
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.SEALCODE_SERVER_PASSWORD
  else process.env.SEALCODE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.SEALCODE_SERVER_USERNAME
  else process.env.SEALCODE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.SEALCODE_SERVER_PASSWORD = auth.password
  Flag.SEALCODE_SERVER_USERNAME = auth.username
  process.env.SEALCODE_SERVER_PASSWORD = auth.password
  process.env.SEALCODE_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  ...
  Flag.SEALCODE_SERVER_USERNAME = auth.username
  delete process.env.SEALCODE_SERVER_PASSWORD
  process.env.SEALCODE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-sealcode-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-sealcode-ticket": "1" }),
  ...
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-sealcode-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  testPty("PTY connect token requires matching directory across mint and connect", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-sealcode-ticket": "1" },
      })
