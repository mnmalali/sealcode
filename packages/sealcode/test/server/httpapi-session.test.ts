...
import { SessionMessage } from "@sealcode-ai/core/session-message"
...
const originalWorkspaces = Flag.SEALCODE_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  Flag.SEALCODE_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session HttpApi", () => {
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-sealcode-directory": test.directory }
    ...
    () =>
      Effect.gen(function* () {
        ...
        const response = yield* request(pathFor(SessionPaths.get, { sessionID: session.id }), {
          headers: { "x-sealcode-directory": test.directory },
        })
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const headers = { "x-sealcode-directory": test.directory, "content-type": "application/json" }
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        Flag.SEALCODE_EXPERIMENTAL_WORKSPACES = true
        ...
        const created = yield* requestJson<Session.Info>(`${SessionPaths.create}?workspace=${workspace.id}`, {
          method: "POST",
          headers: { "x-sealcode-directory": test.directory, "content-type": "application/json" },
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const currentDir = path.join(test.directory, "packages", "sealcode", "src")
