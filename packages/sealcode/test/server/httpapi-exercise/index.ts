 ...
 *
 * The script intentionally isolates `SEALCODE_DB` before importing modules that touch
...
void (await import("@sealcode-ai/core/util/log")).init({ print: false })
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "sealcode.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "sealcode.jsonc")).text(),
          )
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-sealcode-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-sealcode-directory")
  }),
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "sealcode", model: "test" })}`,
      headers: ctx.headers(),
    }))
