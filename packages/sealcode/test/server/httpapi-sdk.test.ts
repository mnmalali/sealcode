...
type Sdk = ReturnType<typeof createSealcodeClient>
...
function app(serverPath: ServerPath, input?: { password?: string; username?: string }) {
  Flag.SEALCODE_SERVER_PASSWORD = input?.password
  Flag.SEALCODE_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return AppFileSystem.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".sealcode", "skills", "project-rest-skill", "SKILL.md"),
      `---
      ...
      Effect.gen(function* () {
        ...
        const bad = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("sealcode", "wrong") },
          }).file.read({ path: "hello.txt" }),
        )
        const good = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("sealcode", "secret") },
          }).file.read({ path: "hello.txt" }),
        )
