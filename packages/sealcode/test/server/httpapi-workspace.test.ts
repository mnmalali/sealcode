...
function request(path: string, directory: string, init: RequestInit = {}) {
  return Effect.promise(() => {
    ...
    headers.set("x-sealcode-directory", directory)
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-sealcode-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://github.com/mnmalali/sealcode/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-sealcode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-sealcode-workspace")
      } finally {
