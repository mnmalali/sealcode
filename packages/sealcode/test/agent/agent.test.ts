...
it.instance("plan agent denies edits except .sealcode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".sealcode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".sealcode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.SEALCODE_TEST_HOME
      process.env.SEALCODE_TEST_HOME = test.directory
