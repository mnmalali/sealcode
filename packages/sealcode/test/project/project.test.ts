...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const sealcodeFile = path.join(tmp, ".git", "sealcode")
      expect(yield* Effect.promise(() => Bun.file(sealcodeFile).exists())).toBe(false)
    }),
  )
  ...
  it.live("should handle git repository with commits", () =>
    Effect.gen(function* () {
      ...
      const sealcodeFile = path.join(tmp, ".git", "sealcode")
      expect(yield* Effect.promise(() => Bun.file(sealcodeFile).exists())).toBe(true)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "sealcode")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "sealcode")
      const wrongCache = path.join(parentDir, ".git", "sealcode")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "sealcode")
      const cacheB = path.join(bareB, "sealcode")
      const wrongCache = path.join(parentDir, ".git", "sealcode")
