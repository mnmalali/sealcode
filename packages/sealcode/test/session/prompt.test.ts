...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<Config.Info>) {
  yield* writeText(
    ...
    JSON.stringify({ $schema: "https://github.com/mnmalali/sealcode/config.json", ...config }),
  )
})
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* llm.text("packages/sealcode/src/session/processor.ts")
  ...
  () =>
    Effect.gen(function* () {
      ...
      const other = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        model: { providerID: ProviderID.make("sealcode"), modelID: ModelID.make("kimi-k2.5-free") },
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })
