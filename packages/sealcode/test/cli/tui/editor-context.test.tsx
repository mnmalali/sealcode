...
const originalSealcodePort = process.env.SEALCODE_EDITOR_SSE_PORT

afterEach(() => {
  process.env.CLAUDE_CODE_SSE_PORT = originalClaudePort
  process.env.SEALCODE_EDITOR_SSE_PORT = originalSealcodePort
})
...
test("useEditorContext reconnect switches editor server by session directory", async () => {
  ...
  process.env.SEALCODE_EDITOR_SSE_PORT = undefined
...
test("useEditorContext connects with SEALCODE_EDITOR_SSE_PORT", async () => {
  await using tmp = await tmpdir()
  process.env.CLAUDE_CODE_SSE_PORT = undefined
  process.env.SEALCODE_EDITOR_SSE_PORT = "4020"
