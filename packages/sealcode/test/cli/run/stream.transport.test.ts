...
import { SealcodeClient, type GlobalEvent } from "@sealcode-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<SealcodeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<SealcodeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<SealcodeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<SealcodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<SealcodeClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://sealcode.test"),
...
function sdk(
  input: {
    ...
    subscribe?: SealcodeClient["event"]["subscribe"]
    globalEvent?: SealcodeClient["global"]["event"]
    promptAsync?: SealcodeClient["session"]["promptAsync"]
    status?: SealcodeClient["session"]["status"]
    messages?: SealcodeClient["session"]["messages"]
    children?: SealcodeClient["session"]["children"]
    permissions?: SealcodeClient["permission"]["list"]
    questions?: SealcodeClient["question"]["list"]
  } = {},
) {
  const client = new SealcodeClient()

  const subscribe: SealcodeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: SealcodeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: SealcodeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: SealcodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: SealcodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: SealcodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: SealcodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: SealcodeClient["question"]["list"] = input.questions ?? (() => ok([]))
