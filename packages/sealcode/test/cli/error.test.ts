...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/sealcode.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/sealcode.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/sealcode.jsonc", dir: ".sealcode", suggestion: "sealcode" },
        expected:
          'Directory ".sealcode" in /tmp/sealcode.jsonc is not valid. Rename the directory to "sealcode" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/sealcode.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/sealcode.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.github.com/mnmalali/sealcode/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.github.com/mnmalali/sealcode/auth/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `sealcode models` to list available models",
      "Or check your config (sealcode.json) provider/model names",
    ].join("\n")
