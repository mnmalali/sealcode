...
import type * as ModelsDev from "@sealcode-ai/core/models"
...
export function options(input: {
  ...
  if (
    ...
    (input.model.providerID === "sealcode" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
  ) {
  ...
  if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
    ...
    if (input.model.providerID.startsWith("sealcode")) {
      result["promptCacheKey"] = input.sessionID
