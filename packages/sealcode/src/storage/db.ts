...
declare const SEALCODE_MIGRATIONS: { sql: string; timestamp: number; name: string }[] | undefined
...
export function getChannelPath(flags: Pick<DatabaseFlags, "disableChannelDb"> = readRuntimeFlags()) {
  if (["latest", "beta", "prod"].includes(InstallationChannel) || flags.disableChannelDb)
    return path.join(Global.Path.data, "sealcode.db")
  const safe = InstallationChannel.replace(/[^a-zA-Z0-9._-]/g, "-")
  return path.join(Global.Path.data, `sealcode-${safe}.db`)
}

export const getPath = (flags?: Pick<DatabaseFlags, "disableChannelDb">) => {
  if (Flag.SEALCODE_DB) {
    if (Flag.SEALCODE_DB === ":memory:" || path.isAbsolute(Flag.SEALCODE_DB)) return Flag.SEALCODE_DB
    return path.join(Global.Path.data, Flag.SEALCODE_DB)
  }
...
export const Client = Object.assign(
  (flags: DatabaseFlags = readRuntimeFlags()): Client => {
    ...
    const entries =
      typeof SEALCODE_MIGRATIONS !== "undefined"
        ? SEALCODE_MIGRATIONS
        : migrations(path.join(import.meta.dirname, "../../migration"))
    if (entries.length > 0) {
      log.info("applying migrations", {
        count: entries.length,
        mode: typeof SEALCODE_MIGRATIONS !== "undefined" ? "bundled" : "dev",
      })
