...
export class Service extends Context.Service<Service, Interface>()("@sealcode/Project") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fakeVcs = Schema.decodeUnknownSync(Schema.optional(ProjectVcs))(Flag.SEALCODE_FAKE_VCS)
    ...
    const readCachedProjectId = Effect.fnUntraced(function* (dir: string) {
      return yield* fs.readFileString(pathSvc.join(dir, "sealcode")).pipe(
        Effect.map((x) => x.trim()),
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const data: DiscoveryResult = yield* Effect.gen(function* () {
        ...
        if (!id) {
          ...
          if (id) {
            yield* fs.writeFileString(pathSvc.join(common, "sealcode"), id).pipe(Effect.ignore)
          }
        }
