...
import { ModelsDev } from "@sealcode-ai/core/models"
...
const AGENT_USERNAME = "sealcode-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/sealcode.yml"
    ...
    yield* Effect.promise(async () => {
      {
        ...
        function printNextSteps() {
          ...
          prompts.outro(
            [
              ...
              "",
              "   Learn more about the GitHub agent - https://github.com/mnmalali/sealcode/docs/github/#usage-examples",
            ].join("\n"),
          )
        }
        ...
        async function installGitHubApp() {
          ...
          // Open browser
          const url = "https://github.com/apps/sealcode-agent"
          const command =
          ...
          async function getInstallation() {
            return await fetch(
              `https://api.github.com/mnmalali/sealcode/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
            )
              .then((res) => res.json())
...
jobs:
  sealcode:
    if: |
      ...
      startsWith(github.event.comment.body, '/sealcode')
    ...
    steps:
      ...
      - name: Run sealcode
        uses: mnmalali/sealcode/github@latest${envStr}
        with:
...
export const GithubRunCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.github.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.github.com/mnmalali/sealcode" : "https://github.com/mnmalali/sealcode"
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.github.com/mnmalali/sealcode"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/sealcode,/oc")
          .split(",")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("sealcode-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `sealcode/${type}-${hex}-${timestamp}`
        }
        return `sealcode/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/sealcode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[sealcode session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the sealcode infrastructure after your response",
