// CLI entry point for `sealcode run`.
//
// Handles three modes:
...
//      sealcode server and runs interactive mode against it.
//
...
import { createSealcodeClient, type SealcodeClient, type ToolPart } from "@sealcode-ai/sdk/v2"
...
type ModelInput = Parameters<SealcodeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run sealcode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running sealcode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to SEALCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to SEALCODE_SERVER_USERNAME or 'sealcode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: SealcodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: SealcodeClient, sessionID: string) {
      ...
      async function current(sdk: SealcodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: SealcodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: SealcodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: SealcodeClient) {
        ...
        async function loop(client: SealcodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createSealcodeClient({
        baseUrl: "http://sealcode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
