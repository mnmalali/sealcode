...
import { createSealcodeClient } from "@sealcode-ai/sdk/v2"
...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "sealcode.mode": input.resolveSession ? "local" : "attach",
      "sealcode.initial_input": !!input.initialInput,
      "sealcode.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "sealcode.directory": ctx.directory,
        "sealcode.resume": ctx.resume === true,
        "sealcode.agent.name": state.agent,
        "sealcode.model.provider": state.model?.providerID,
        "sealcode.model.id": state.model?.modelID,
        "sealcode.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "sealcode.model.provider": model.providerID,
            "sealcode.model.id": model.modelID,
            "sealcode.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "sealcode.model.id": state.model?.modelID,
                "sealcode.model.variant": state.activeVariant,
                "sealcode.prompt.chars": prompt.text.length,
                "sealcode.prompt.parts": prompt.parts.length,
                "sealcode.prompt.include_files": includeFiles,
                "sealcode.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "sealcode.directory": input.directory,
      "sealcode.initial_input": !!input.initialInput,
      "sealcode.demo": input.demo,
    },
    async () => {
