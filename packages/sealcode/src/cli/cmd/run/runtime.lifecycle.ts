...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "sealcode.agent.name": input.agent,
      "sealcode.directory": input.directory,
      "sealcode.first": input.first,
      "sealcode.model.provider": input.model?.providerID,
      "sealcode.model.id": input.model?.modelID,
      "sealcode.model.variant": input.variant,
      ...
      try {
        ...
        const close = async (next: {
          ...
          return withRunSpan(
            "RunLifecycle.close",
            {
              "sealcode.show_exit": next.showExit,
