...
import sealcode from "./theme/sealcode.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@sealcode-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "sealcode")
        draft.active = typeof active === "string" ? active : "sealcode"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.sealcode, store.mode)
    })
