...
import { Flock } from "@sealcode-ai/core/util/flock"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".sealcode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".sealcode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".sealcode", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Resolved; dispose?: () => void }) {
  ...
  try {
    ...
    const records = Flag.SEALCODE_PURE ? [] : (config.plugin_origins ?? [])
    if (Flag.SEALCODE_PURE && config.plugin_origins?.length) {
