...
const TUI_SCHEMA_URL = "https://github.com/mnmalali/sealcode/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from sealcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const sealcode = await sealcodeFiles(input)
  for (const file of sealcode) {
...
async function sealcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "sealcode"),
    ...(await Filesystem.findUp(["sealcode.json", "sealcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "sealcode"))
  }
  if (Flag.SEALCODE_CONFIG) files.push(Flag.SEALCODE_CONFIG)
