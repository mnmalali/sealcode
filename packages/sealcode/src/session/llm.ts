...
export class Service extends Context.Service<Service, Interface>()("@sealcode/LLM") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("LLM.run")(function* (input: StreamRequest) {
      ...
      // from the workflow service are executed via sealcode's tool system
      ...
      const sealcodeProjectID = input.model.providerID.startsWith("sealcode")
      ...
      return streamText({
        ...
        abortSignal: input.abort,
        headers: {
          ...(input.model.providerID.startsWith("sealcode")
            ? {
                "x-sealcode-project": sealcodeProjectID,
                "x-sealcode-session": input.sessionID,
                "x-sealcode-request": input.user.id,
                "x-sealcode-client": flags.client,
