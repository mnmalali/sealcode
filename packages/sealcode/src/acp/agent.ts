...
import type { AssistantMessage, Event, SealcodeClient, SessionMessageResponse, ToolPart } from "@sealcode-ai/sdk/v2"
...
export function init({ sdk: _sdk }: { sdk: SealcodeClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `sealcode auth login` in the terminal",
      name: "Login with sealcode",
      id: "sealcode-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // Sealcode stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const sealcodeProvider = providers.find((p) => p.id === "sealcode")
  if (sealcodeProvider) {
    const [best] = Provider.sort(Object.values(sealcodeProvider.models))
    if (best) {
      return {
