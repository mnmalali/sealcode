...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for sealcode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal sealcode sessions
...
sealcode acp --cwd /path/to/project
```
...
```bash
SEALCODE_ENABLE_QUESTION_TOOL=1 sealcode acp
```
...
- **Terminal Integration**: Full terminal support via sealcode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | sealcode acp
```
...
ACP sessions map cleanly to sealcode's internal session model:
