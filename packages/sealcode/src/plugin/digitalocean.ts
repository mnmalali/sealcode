import type { Hooks, PluginInput } from "@sealcode-ai/plugin"
import type { Model } from "@sealcode-ai/sdk/v2"
...
const MAK_NAME_PREFIX = "sealcode-oauth"
...
<html>
  <head>
    <meta charset="utf-8" />
    <title>Sealcode - DigitalOcean Authorization</title>
    <style>
    ...
    <script>
      (async function() {
        ...
        try {
          ...
          msgEl.textContent = "You can close this window and return to Sealcode."
...
async function createModelAccessKey(bearer: string): Promise<ApiKeyInfo> {
  ...
  const res = await fetch(`${DO_API_BASE}/v2/gen-ai/models/api_keys`, {
    ...
    headers: {
      ...
      "User-Agent": `sealcode/${InstallationVersion}`,
    },
      ...
      methods: [
        {
          ...
          async authorize() {
            ...
            return {
              ...
              instructions:
                "Sign in to DigitalOcean in your browser. Sealcode will create a Model Access Key named sealcode-oauth-* and load your Inference Routers. Re-run /connect to refresh routers later.",
