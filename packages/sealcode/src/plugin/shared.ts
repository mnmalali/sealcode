...
export const DEPRECATED_PLUGIN_PACKAGES = ["sealcode-openai-codex-auth", "sealcode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, sealcodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(sealcodeVersion) || semver.major(sealcodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.sealcode
  if (typeof range !== "string") return
  if (!semver.satisfies(sealcodeVersion, range)) {
    throw new Error(`Plugin requires sealcode ${range} but running ${sealcodeVersion}`)
  }
}
