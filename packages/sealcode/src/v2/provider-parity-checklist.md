...
This tracks legacy provider behavior from `packages/sealcode/src/provider/provider.ts` that still needs to be ported into the v2 provider plugins under `packages/sealcode/src/v2/plugin/provider/`. Keep entries checked only when v2 has equivalent behavior or when the item is intentionally skipped.
...
- [x] Sealcode unauthenticated paid-model filtering.
- [x] Sealcode public API key fallback.
...
- [x] Sealcode small model priority. Replaced by cheapest output cost selection.
