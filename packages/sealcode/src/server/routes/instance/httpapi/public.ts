...
import { SealcodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for sealcode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(SealcodeHttpApi)
...
export const PublicApi = SealcodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "sealcode",
    version: "1.0.0",
    description: "sealcode api",
