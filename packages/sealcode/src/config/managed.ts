...
const MANAGED_PLIST_DOMAIN = "ai.sealcode.managed"

// Keys injected by macOS/MDM into the managed plist that are not Sealcode config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/sealcode"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "sealcode")
    default:
      return "/etc/sealcode"
  }
}
...
export function managedConfigDir() {
  return process.env.SEALCODE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
