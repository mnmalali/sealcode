...
You can add external tools to Sealcode using the _Model Context Protocol_, or MCP. Sealcode supports both local and remote servers.
...
You can define MCP servers in your [Sealcode Config](https://github.com/mnmalali/sealcode/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/sealcode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Sealcode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Sealcode will:
...
If the server requires authentication, Sealcode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `sealcode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Sealcode will store the tokens securely in `~/.local/share/sealcode/mcp-auth.json`.
...
Your MCPs are available as tools in Sealcode, alongside built-in tools. So you can manage them through the Sealcode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Sealcode to your Sentry account.
