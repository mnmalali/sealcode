---
title: LSP Servers
description: Sealcode integrates with your LSP servers.
---

Sealcode can integrate with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

Sealcode comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `SEALCODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and sealcode opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your sealcode config.
