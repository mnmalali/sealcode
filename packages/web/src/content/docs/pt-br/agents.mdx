...
Existem dois tipos de agentes no sealcode; agentes primários e subagentes.
...
sealcode vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Sealcode vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Sealcode vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Sealcode, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `sealcode.json`:
...
- Global: `~/.config/sealcode/agents/`
- Por projeto: `.sealcode/agents/`

```markdown title="~/.config/sealcode/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o sealcode usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do sealcode quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do sealcode usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Sealcode Zen](/docs/zen), você usaria `sealcode/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="sealcode.json" {3-6,9-12}
{
  "$schema": "https://github.com/mnmalali/sealcode/config.json",
  "tools": {
    "write": true,
...
```json title="sealcode.json" {3-5,8-10}
{
  "$schema": "https://github.com/mnmalali/sealcode/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `sealcode models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/mnmalali/sealcode).
:::
...
```markdown title="~/.config/sealcode/agents/docs-writer.md"
---
...
```markdown title="~/.config/sealcode/agents/security-auditor.md"
---
