...
O Sealcode Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

:::note
O Sealcode Go está atualmente em beta.
:::

O Go funciona como qualquer outro provedor no Sealcode. Você assina o Sealcode Go e
...
O Sealcode Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O Sealcode Go funciona como qualquer outro provedor no Sealcode.

1. Você entra no **<a href={console}>Sealcode Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Sealcode Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Sealcode Go.
:::
...
O Sealcode Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Sealcode
usa o formato `sealcode-go/<model-id>`. Por exemplo, para o Kimi K2.6, você usaria
`sealcode-go/kimi-k2.6` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Sealcode também.
