...
As ferramentas permitem que o LLM execute ações em sua base de código. O sealcode vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no sealcode.
...
:::note
Esta ferramenta está disponível apenas quando `SEALCODE_EXPERIMENTAL_LSP_TOOL=true` (ou `SEALCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor sealcode ou quando a variável de ambiente `SEALCODE_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o sealcode:
