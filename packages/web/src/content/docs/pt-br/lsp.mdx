...
description: O sealcode integra-se aos seus servidores LSP.
---

O sealcode integra-se ao seu Protocolo de Servidor de Linguagem (LSP) para ajudar o LLM a interagir com sua base de código. Ele usa diagnósticos para fornecer feedback ao LLM.

---

## Integrados

O sealcode vem com vários servidores LSP integrados para linguagens populares:
...
:::note
Você pode desabilitar os downloads automáticos do servidor LSP definindo a variável de ambiente `SEALCODE_DISABLE_LSP_DOWNLOAD` como `true`.
:::
...
Quando o sealcode abre um arquivo, ele:
...
Você pode personalizar os servidores LSP através da seção `lsp` na sua configuração do sealcode.
