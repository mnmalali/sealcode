...
Com o sealcode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o sealcode usa nosso próprio tema `sealcode`.
...
O sealcode vem com vários temas embutidos.
...
- Querem que o sealcode corresponda à aparência do seu terminal
...
O sealcode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/sealcode/themes/*.json` ou `$XDG_CONFIG_HOME/sealcode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.sealcode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.sealcode/themes/*.json`
