---
title: Plugins
description: Escreva seus próprios plugins para estender o sealcode.
---

Plugins permitem que você estenda o sealcode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do sealcode.
...
- `.sealcode/plugins/` - Plugins em nível de projeto
- `~/.config/sealcode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/sealcode/node_modules/`.
...
1. Configuração global (`~/.config/sealcode/sealcode.json`)
2. Configuração do projeto (`sealcode.json`)
3. Diretório de plugins global (`~/.config/sealcode/plugins/`)
4. Diretório de plugins do projeto (`.sealcode/plugins/`)
...
O sealcode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do sealcode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o sealcode.
...
:::note
Se você estiver usando o aplicativo desktop sealcode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o sealcode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao sealcode:
...
O helper `tool` cria uma ferramenta personalizada que o sealcode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o sealcode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://github.com/mnmalali/sealcode/docs/sdk) para detalhes.
