...
As habilidades do agente permitem que o sealcode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O sealcode pesquisa nesses locais:

- Configuração do projeto: `.sealcode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/sealcode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o sealcode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.sealcode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/sealcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.sealcode/skills/git-release/SKILL.md` assim:
...
O sealcode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `sealcode.json`:
...
**Para agentes embutidos** (em `sealcode.json`):
