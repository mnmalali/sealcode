---
title: Servidor
description: Interaja com o servidor sealcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `sealcode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente sealcode pode usar.
...
```bash
sealcode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `sealcode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
sealcode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `SEALCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `sealcode`, ou defina `SEALCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `sealcode serve` quanto ao `sealcode web`.

```bash
SEALCODE_SERVER_PASSWORD=your-password sealcode serve
```
...
Quando você executa `sealcode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor sealcode para interagir com o sealcode programaticamente.
:::

Essa arquitetura permite que o sealcode suporte múltiplos clientes e permite que você interaja com o sealcode programaticamente.

Você pode executar `sealcode serve` para iniciar um servidor autônomo. Se você tiver o TUI do sealcode em execução, `sealcode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do sealcode [IDE](/docs/ide).
...
O servidor sealcode expõe as seguintes APIs.
