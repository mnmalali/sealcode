...
Le skill degli agenti permettono a Sealcode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Sealcode cerca in queste posizioni:

- Config di progetto: `.sealcode/skills/<name>/SKILL.md`
- Config globale: `~/.config/sealcode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Sealcode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.sealcode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/sealcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.sealcode/skills/git-release/SKILL.md` cosi':
...
Sealcode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `sealcode.json`:
...
**Per agenti integrati** (in `sealcode.json`):
