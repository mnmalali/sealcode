...
Sealcode Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

:::note
Sealcode Go è attualmente in beta.
:::

Go funziona come qualsiasi altro provider in Sealcode. Ti abboni a Sealcode Go e
...
Sealcode Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

Sealcode Go funziona come qualsiasi altro provider in Sealcode.

1. Accedi a **<a href={console}>Sealcode Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Sealcode Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Sealcode Go.
:::
...
Sealcode Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Sealcode config
utilizza il formato `sealcode-go/<model-id>`. Ad esempio, per Kimi K2.6, useresti
`sealcode-go/kimi-k2.6` nella tua configurazione.
...
Abbiamo creato Sealcode Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Sealcode.
