---
title: Plugin
description: Scrivi plugin per estendere Sealcode.
---

I plugin ti permettono di estendere Sealcode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Sealcode.
...
- `.sealcode/plugins/` - plugin a livello progetto
- `~/.config/sealcode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/sealcode/node_modules/`.
...
1. Config globale (`~/.config/sealcode/sealcode.json`)
2. Config di progetto (`sealcode.json`)
3. Directory plugin globale (`~/.config/sealcode/plugins/`)
4. Directory plugin di progetto (`.sealcode/plugins/`)
...
Sealcode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di sealcode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere sealcode.
...
:::note
Se usi l'app desktop di Sealcode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a sealcode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a sealcode:
...
L'helper `tool` crea uno strumento personalizzato che sealcode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in sealcode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://github.com/mnmalali/sealcode/docs/sdk) per i dettagli.
