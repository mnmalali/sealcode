...
Puoi aggiungere strumenti esterni a Sealcode usando il _Model Context Protocol_, o MCP. Sealcode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Sealcode Config](https://github.com/mnmalali/sealcode/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/sealcode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Sealcode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Sealcode:
...
Se il server richiede autenticazione, Sealcode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `sealcode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Sealcode memorizzera' i token in modo sicuro in `~/.local/share/sealcode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Sealcode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Sealcode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Sealcode al tuo account Sentry.
