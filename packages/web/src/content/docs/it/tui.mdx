---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Sealcode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Sealcode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Sealcode avvia la TUI per la directory corrente.
...
Quando usi la TUI di Sealcode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Sealcode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Sealcode. _Alias_: `/quit`, `/q`
...
Questo è separato da `sealcode.json`, che configura il comportamento di server/runtime.
...
Usa `SEALCODE_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
