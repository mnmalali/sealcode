...
Con Sealcode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, Sealcode usa il tema `sealcode`.
...
Sealcode include diversi temi integrati.
...
- Vuole che Sealcode corrisponda all'aspetto del terminale
...
Sealcode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/sealcode/themes/*.json` o `$XDG_CONFIG_HOME/sealcode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.sealcode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.sealcode/themes/*.json`
