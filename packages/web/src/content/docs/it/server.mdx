---
title: Server
description: Interagisci con il server sealcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `sealcode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client sealcode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `sealcode.local` |
...
Imposta `SEALCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `sealcode`, oppure imposta `SEALCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `sealcode serve` sia per `sealcode web`.
...
Quando esegui `sealcode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server sealcode per interagire con sealcode in modo programmatico.
:::

Questa architettura permette a sealcode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `sealcode serve` per avviare un server standalone. Se la TUI di sealcode e gia in esecuzione, `sealcode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Sealcode.
...
Il server sealcode espone le seguenti API.
