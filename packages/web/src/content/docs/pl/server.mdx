---
title: Serwer
description: Interakcja z serwerem sealcode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `sealcode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient sealcode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `sealcode.local` |
...
Ustaw `SEALCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `sealcode` lub ustaw `SEALCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `sealcode serve`, jak i `sealcode web`.
...
Kiedy uruchomisz `sealcode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera sealcode do programowej interakcji z sealcode.
:::

Ta architektura umożliwia obsługę wielu klientów przez sealcode i programową interakcję z kodem otwartym.

Możesz uruchomić `sealcode serve`, aby uruchomić samodzielny serwer. Jeśli masz
sealcode TUI działa, `sealcode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki sealcode [IDE](/docs/ide).
...
Serwer sealcode udostępnia następujące interfejsy API.
