...
Możesz dodać dodatkowe informacje do sealcode za pomocą _Model Context Protocol_, w skrócie MCP. sealcode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [sealcode Config](https://github.com/mnmalali/sealcode/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/sealcode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
sealcode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, sealcode:
...
Jeśli serwer wymaga uwierzytelnienia, sealcode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `sealcode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji sealcode bezpieczne przechowa tokeny w `~/.local/share/sealcode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w sealcode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie sealcode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć sealcode z kontem Sentry.
