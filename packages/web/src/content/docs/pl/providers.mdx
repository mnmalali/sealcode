---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w sealcode.
---

import config from "../../../../config.mjs"
export const console = config.console

sealcode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji sealcode.
...
w `~/.local/share/sealcode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim sealcode
config.
...
Sealcode Zen to lista modeli dostarczonych przez zespół sealcode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z sealcode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Sealcode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Sealcode Zen` i przejdź do [github.com/mnmalali/sealcode/auth](https://github.com/mnmalali/sealcode/zen).
...
Działa jak każdy inny dostawca w sealcode i jest całkowicie opcjonalny w użyciu.

---

## Sealcode Go

Sealcode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół sealcode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z sealcode.

1. Uruchom polecenie `/connect` w TUI, wybierz `Sealcode Go` i przejdź do [github.com/mnmalali/sealcode/auth](https://github.com/mnmalali/sealcode/zen).
...
Aby używać Amazon Bedrock z sealcode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania sealcode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `sealcode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w sealcode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować sealcode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod sealcode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji sealcode.
...
sealcode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować sealcode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`sealcode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem sealcode:
...
Aby używać Google Vertex AI z sealcode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania sealcode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą sealcode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [sealcode-helicone-session](https://github.com/H2Shami/sealcode-helicone-session), aby automatycznie rejestrować każdą konwersację sealcode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja sealcode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować sealcode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować sealcode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować sealcode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem sealcode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/sealcode).
:::
...
Aby korzystać z Ollama Cloud z sealcode:
...
3. Skopiuj klucz API do użycia w sealcode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w sealcode musisz pobrać informacje o modelu lokalnie:
...
Sealcode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół sealcode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Sealcode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Sealcode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API sealcode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji sealcode.
...
5. Możesz je także dostosować za pomocą konfiguracji sealcode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z sealcode:
...
Możesz także dostosować modele za pomocą konfiguracji sealcode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z sealcode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `sealcode.json` w katalogu projektu:
...
Pola `limit` pozwalają sealcode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `sealcode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację sealcode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji sealcode.
