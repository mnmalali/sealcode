---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć sealcode.
---

Wtyczki umożliwiające wyłączenie sealcode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania sealcode.
...
- `.sealcode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/sealcode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/sealcode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/sealcode/sealcode.json`)
2. Konfiguracja projektu (`sealcode.json`)
3. Globalny katalog wtyczek (`~/.config/sealcode/plugins/`)
4. Katalog wtyczek projektu (`.sealcode/plugins/`)
...
sealcode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z sealcode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do sealcode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej sealcode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj sealcode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do sealcode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod sealcode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://github.com/mnmalali/sealcode/docs/sdk).
