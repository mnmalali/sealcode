...
Aby debugować problemy z sealcode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\sealcode\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `sealcode --log-level DEBUG`.

---

## Przechowywanie danych

sealcode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/sealcode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\sealcode`
...
sealcode Desktop uruchamia w tle lokalny serwer sealcode (sidecar `sealcode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Sealcode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/sealcode/sealcode.jsonc` (lub `~/.config/sealcode/sealcode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/sealcode/sealcode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\sealcode\sealcode.jsonc`
...
sealcode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/sealcode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\sealcode\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby sealcode mógł ją odbudować.

1. Całkowicie zamknij sealcode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/sealcode`
- **Linux**: usuń `~/.cache/sealcode` (lub uruchom `rm -rf ~/.cache/sealcode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\sealcode`

3. Uruchom ponownie sealcode Desktop.

---

### Napraw problemy z połączeniem z serwerem

sealcode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `sealcode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `SEALCODE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `SEALCODE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows sealcode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami sealcode.

---

### Brak powiadomień

sealcode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla sealcode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie sealcode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji sealcode Desktop):

- `sealcode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `sealcode.global.dat` i `sealcode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z sealcode:
...
3. Upewnij się, że masz najnowszą wersję z `sealcode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `sealcode models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\sealcode`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. sealcode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\sealcode`

2. Uruchom ponownie kod sealcode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to sealcode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
sealcode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
