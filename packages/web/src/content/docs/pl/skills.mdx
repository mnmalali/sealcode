...
Umiejętności agenta pozwalają sealcode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
sealcode przeszukuje te lokalizacje:

- Project config: `.sealcode/skills/<name>/SKILL.md`
- Global config: `~/.config/sealcode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu sealcode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.sealcode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/sealcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.sealcode/skills/git-release/SKILL.md` w ten sposób:
...
sealcode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `sealcode.json`:
...
**Dla agentów wbudowanych** (w `sealcode.json`):
