...
Dzięki sealcode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie sealcode używa naszego własnego motywu `sealcode`.
...
sealcode ma kilka wbudowanych motywów.
...
- Chcą, aby Sealcode pasował do wyglądu ich terminala
...
sealcode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/sealcode/themes/*.json` lub `$XDG_CONFIG_HOME/sealcode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.sealcode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.sealcode/themes/*.json`
