...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Sealcode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Sealcode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `SEALCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `SEALCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Sealcode lub gdy zmienna środowiskowa `SEALCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Sealcode:
