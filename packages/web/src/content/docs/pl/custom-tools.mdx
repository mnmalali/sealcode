...
Narzędzia specjalistyczne to funkcje, które tworzysz i które LLM może wywoływać podczas rozmów. Współpracują one z [wbudowanymi narzędziami](/docs/tools) sealcode, takimi jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.sealcode/tools/` swojego projektu.
- Lub globalnie, umieszczając je w `~/.config/sealcode/tools/`.
