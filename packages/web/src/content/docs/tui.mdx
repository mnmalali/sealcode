---
title: TUI
description: Using the Sealcode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Sealcode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Sealcode starts the TUI for the current directory.
...
When using the Sealcode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Sealcode. Allows you to select from available providers and add their API keys.
...
Exit Sealcode. _Aliases_: `/quit`, `/q`
...
This is separate from `sealcode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Sealcode waits after the leader key. Defaults to `2000`.
...
Use `SEALCODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `sealcode.default`.
