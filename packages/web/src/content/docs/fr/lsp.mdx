---
title: Serveurs LSP
description: Sealcode s'intègre à vos serveurs LSP.
---

Sealcode s'intègre via le protocole LSP (Language Server Protocol) pour aider le LLM à interagir avec votre base de code. Il utilise des diagnostics pour fournir des commentaires au LLM.

---

## Serveurs intégrés

Sealcode est livré avec plusieurs serveurs LSP intégrés pour les langages populaires :
...
:::note
Vous pouvez désactiver les téléchargements automatiques du serveur LSP en définissant la variable d'environnement `SEALCODE_DISABLE_LSP_DOWNLOAD` sur `true`.
:::
...
Lorsque sealcode ouvre un fichier, il :
...
Vous pouvez personnaliser les serveurs LSP via la section `lsp` de votre configuration sealcode.
