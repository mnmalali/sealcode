...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Sealcode.
...
Voici plusieurs modèles qui fonctionnent bien avec Sealcode, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Sealcode.

```json title="sealcode.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Sealcode Zen](/docs/zen), vous utiliserez `sealcode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="sealcode.jsonc" {7-12,19-24}
{
  "$schema": "https://github.com/mnmalali/sealcode/config.json",
  "provider": {
    "openai": {
...
```jsonc title="sealcode.jsonc" {6-21}
{
  "$schema": "https://github.com/mnmalali/sealcode/config.json",
  "provider": {
    "sealcode": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Sealcode est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Sealcode est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="sealcode.jsonc" {7-18}
{
  "$schema": "https://github.com/mnmalali/sealcode/config.json",
  "provider": {
    "openai": {
...
Lorsque Sealcode démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Sealcode.
