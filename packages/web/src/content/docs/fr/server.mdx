---
title: Serveur
description: Interagissez avec le serveur sealcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `sealcode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client sealcode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `sealcode.local` |
...
Définissez `SEALCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `sealcode`, ou définissez `SEALCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `sealcode serve` et à `sealcode web`.
...
Lorsque vous exécutez `sealcode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur sealcode pour interagir avec sealcode par programmation.
:::

Cette architecture permet à sealcode de prendre en charge plusieurs clients et vous permet d'interagir avec sealcode par programme.

Vous pouvez exécuter `sealcode serve` pour démarrer un serveur autonome. Si vous avez le
sealcode TUI en cours d'exécution, `sealcode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Sealcode [IDE](/docs/ide).
...
Le serveur sealcode expose les API suivantes.
