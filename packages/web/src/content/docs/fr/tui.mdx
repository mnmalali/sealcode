---
title: TUI
description: Utilisation de l’interface utilisateur du terminal Sealcode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Sealcode fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de Sealcode démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le Sealcode TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à Sealcode. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Ceci est séparé de `sealcode.json`, qui configure le comportement du serveur/d'exécution.
...
Utilisez `SEALCODE_TUI_CONFIG` pour charger un chemin de configuration TUI personnalisé.
