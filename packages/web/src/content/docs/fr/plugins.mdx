---
title: Plugins
description: Écrivez vos propres plugins pour étendre Sealcode.
---

Les plugins vous permettent d'étendre Sealcode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Sealcode.
...
- `.sealcode/plugins/` - Plugins au niveau du projet
- `~/.config/sealcode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/sealcode/node_modules/`.
...
1. Configuration globale (`~/.config/sealcode/sealcode.json`)
2. Configuration du projet (`sealcode.json`)
3. Répertoire global des plugins (`~/.config/sealcode/plugins/`)
4. Répertoire des plugins du projet (`.sealcode/plugins/`)
...
Sealcode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK sealcode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre sealcode.
...
:::note
Si vous utilisez l'application de bureau Sealcode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez sealcode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à sealcode :
...
L'assistant `tool` crée un outil personnalisé que sealcode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour sealcode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://github.com/mnmalali/sealcode/docs/sdk) pour plus de détails.
