...
Avec Sealcode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Sealcode utilise notre propre thème `sealcode`.
...
Sealcode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Sealcode corresponde à l'apparence de leur terminal
...
Sealcode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/sealcode/themes/*.json` ou `$XDG_CONFIG_HOME/sealcode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.sealcode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.sealcode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/sealcode/themes
vim ~/.config/sealcode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .sealcode/themes
vim .sealcode/themes/my-theme.json
```
...
{
  "$schema": "https://github.com/mnmalali/sealcode/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
