---
title: SDK
description: Client JS de type sécurisé pour le serveur sealcode.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK sealcode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler sealcode par programme.
...
Créez une instance de sealcode :

```javascript
import { createSealcode } from "@sealcode-ai/sdk"

const { client } = await createSealcode()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `sealcode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${sealcode.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de sealcode, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@sealcode-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*sealcode" },
})
