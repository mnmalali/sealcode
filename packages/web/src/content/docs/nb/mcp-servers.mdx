...
Du kan legge til eksterne verktøy i Sealcode ved å bruke _Model Context Protocol_ eller MCP. Sealcode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Sealcode Config](https://github.com/mnmalali/sealcode/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="sealcode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/sealcode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Sealcode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Sealcode:
...
Hvis serveren krever autentisering, vil Sealcode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `sealcode mcp auth <server-name>`.
...
```bash
sealcode mcp auth my-oauth-server
```
...
```bash
sealcode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Sealcode lagre tokens sikkert i `~/.local/share/sealcode/mcp-auth.json`.
...
sealcode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Sealcode, sammen med innebygde verktøy. Så du kan administrere dem gjennom Sealcode-konfigurasjonen som alle andre verktøy.
...
```json title="sealcode.json" {11, 14-18}
{
  "$schema": "https://github.com/mnmalali/sealcode/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
sealcode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Sealcode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Sealcode-samtale oppført som en separat økt.
