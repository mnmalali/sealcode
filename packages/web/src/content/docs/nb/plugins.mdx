...
description: Skriv dine egne programtillegg for å utvide Sealcode.
---

Plugins lar deg utvide Sealcode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Sealcode.
...
- `.sealcode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/sealcode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/sealcode/sealcode.json`)
2. Prosjektkonfigurasjon (`sealcode.json`)
3. Global plugin-katalog (`~/.config/sealcode/plugins/`)
4. Prosjektpluginkatalog (`.sealcode/plugins/`)
...
Sealcode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Sealcode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Sealcode.
...
:::note
Hvis du bruker Sealcode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Sealcode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Sealcode:
...
`tool`-hjelperen lager et tilpasset verktøy som Sealcode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Sealcode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://github.com/mnmalali/sealcode/docs/sdk) for detaljer.
