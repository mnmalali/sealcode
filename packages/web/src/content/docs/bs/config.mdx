---
title: Konfiguracija
description: Korištenje Sealcode JSON konfiguracije.
---

Možete konfigurirati Sealcode koristeći JSON konfiguracijski fajl.

---

## Format

Sealcode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/sealcode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/sealcode/sealcode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`SEALCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`sealcode.json` u projektu) - postavke specifične za projekat
5. **`.sealcode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`SEALCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.sealcode` i `~/.config/sealcode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/sealcode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Sealcode konfiguraciju u `~/.config/sealcode/sealcode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/sealcode/tui.json`.
...
Dodajte `sealcode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Sealcode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `SEALCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `SEALCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.sealcode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.sealcode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`github.com/mnmalali/sealcode/config.json`**](https://github.com/mnmalali/sealcode/config.json).

TUI konfiguracija koristi [**`github.com/mnmalali/sealcode/tui.json`**](https://github.com/mnmalali/sealcode/tui.json).
...
Koristite `SEALCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `sealcode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `sealcode serve` i `sealcode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Sealcode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `sealcode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Sealcode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Sealcode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/sealcode/agents/` ili `.sealcode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Sealcode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`sealcode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/sealcode/commands/` ili `.sealcode/commands/`. [Saznajte više ovdje](/docs/commands).
...
Sealcode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Sealcode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Sealcode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.sealcode/plugins/` ili `~/.config/sealcode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Sealcode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
