...
Možete dodati vanjske alate u Sealcode koristeći _Model Context Protocol_, ili MCP. Sealcode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Sealcode Config](https://github.com/mnmalali/sealcode/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/sealcode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Sealcode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Sealcode će:
...
Ako server zahtijeva autentifikaciju, Sealcode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `sealcode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Sealcode sigurno čuva tokene u `~/.local/share/sealcode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Sealcode, zajedno s ugrađenim alatima. Možete njima upravljati kroz Sealcode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Sealcode sa vašim Sentry nalogom.
