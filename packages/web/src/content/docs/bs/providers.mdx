---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Sealcode.
---

import config from "../../../../config.mjs"
export const console = config.console

Sealcode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Sealcode konfiguraciji.
...
u `~/.local/share/sealcode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Sealcode
config.
...
Sealcode Zen je lista modela koje je obezbedio Sealcode tim koji su bili
testirano i potvrđeno da dobro radi sa Sealcode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Sealcode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite sealcode i idite na [github.com/mnmalali/sealcode/auth](https://github.com/mnmalali/sealcode/auth).
...
Radi kao i svaki drugi provajder u Sealcode i potpuno je opcionalan za korištenje.

---

## Sealcode Go

Sealcode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Sealcode tim i koji su testirani i verificirani da dobro rade s Sealcode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Sealcode Go` i idite na [github.com/mnmalali/sealcode/auth](https://github.com/mnmalali/sealcode/zen).
...
Da biste koristili Amazon Bedrock s Sealcode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete sealcode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `sealcode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Sealcode.
:::
...
Možete konfigurirati sealcode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi sealcode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju sealcode konfiguraciju.
...
da pohrani token u sealcode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Sealcode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Sealcode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`sealcode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `sealcode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s sealcode:
...
Za korištenje Google Vertex AI s Sealcode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće sealcode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem sealcodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [sealcode-helicone-session](https://github.com/H2Shami/sealcode-helicone-session) da automatski prijavite svaki Sealcode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Sealcode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati sealcode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati sealcode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati sealcode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Sealcode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/sealcode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Sealcode:
...
3. Kopirajte API ključ za korištenje u Sealcode.
...
6. **Važno**: Prije upotrebe modela oblaka u Sealcode, morate lokalno povući informacije o modelu:
...
Sealcode Zen je lista testiranih i verifikovanih modela koju je obezbedio Sealcode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Sealcode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Sealcode Zen**.

```txt
   /connect
```

3. Unesite svoj Sealcode API ključ.
...
Također možete dodati dodatne modele putem vaše sealcode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše sealcode konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju sealcode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s sealcode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `sealcode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Sealcode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `sealcode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Sealcode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj sealcode konfiguraciji.
