...
U Sealcode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Sealcode koristi nasu `sealcode` temu.
...
Sealcode dolazi sa vise ugradenih tema.
...
- Zele da Sealcode odgovara izgledu njihovog terminala
...
Sealcode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/sealcode/themes/*.json` ili `$XDG_CONFIG_HOME/sealcode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.sealcode/themes/*.json`
4. **Trenutni radni direktorij** - `./.sealcode/themes/*.json`
