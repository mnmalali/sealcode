...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Sealcode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Sealcode.
...
:::note
Ovaj alat je dostupan samo kada je `SEALCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `SEALCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Sealcode provajdera ili kada je varijabla `SEALCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Sealcode:
