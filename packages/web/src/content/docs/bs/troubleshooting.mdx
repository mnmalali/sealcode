...
Da biste otklonili probleme s Sealcode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\sealcode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `sealcode --log-level DEBUG`.

---

## Pohrana

Sealcode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/sealcode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\sealcode`
...
Sealcode Desktop pokreće lokalni Sealcode server (`sealcode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Sealcode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/sealcode/sealcode.jsonc` (ili `~/.config/sealcode/sealcode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/sealcode/sealcode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\sealcode\sealcode.jsonc`
...
Sealcode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/sealcode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\sealcode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Sealcode mogao ponovo izgraditi.

1. Potpuno zatvorite Sealcode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/sealcode`
- **Linux**: obrišite `~/.cache/sealcode` (ili pokrenite `rm -rf ~/.cache/sealcode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\sealcode`

3. Ponovo pokrenite Sealcode Desktop.

---

### Rješavanje problema sa vezom na serveru

Sealcode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `sealcode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `SEALCODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `SEALCODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Sealcode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Sealcode karakteristikama.

---

### Obavještenja se ne prikazuju

Sealcode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Sealcode u postavkama vašeg OS-a, i
...
1. Zatvorite Sealcode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Sealcode Desktop aplikacije):

- `sealcode.settings.dat` (URL zadanog servera za desktop)
- `sealcode.global.dat` i `sealcode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `sealcode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\sealcode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Sealcode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\sealcode`

2. Ponovo pokrenite Sealcode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Sealcode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Sealcode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
