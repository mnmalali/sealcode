---
title: Server
description: Komunicirajte s sealcode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `sealcode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji sealcode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `sealcode.local` |
...
Postavite `SEALCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `sealcode`, ili postavite `SEALCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `sealcode serve` i za `sealcode web`.
...
Kada pokrenete `sealcode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite sealcode server za programsku interakciju sa sealcode-om.
:::

Ova arhitektura omogucava sealcode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `sealcode serve` da startate standalone server. Ako je sealcode TUI vec pokrenut, `sealcode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Sealcode [IDE](/docs/ide) pluginovi.
...
sealcode server izlaže sljedece API-je.
