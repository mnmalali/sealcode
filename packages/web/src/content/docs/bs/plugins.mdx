---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Sealcode.
---

Dodaci vam omogućavaju da proširite Sealcode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Sealcode.
...
- `.sealcode/plugins/` - Dodaci na nivou projekta
- `~/.config/sealcode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/sealcode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/sealcode/sealcode.json`)
2. Konfiguracija projekta (`sealcode.json`)
3. Globalni direktorij dodataka (`~/.config/sealcode/plugins/`)
4. Direktorij dodataka projekta (`.sealcode/plugins/`)
...
Sealcode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Sealcode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Sealcode.
...
Spriječite sealcode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u sealcode:
...
Pomoćnik `tool` kreira prilagođeni alat koji sealcode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za sealcode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://github.com/mnmalali/sealcode/docs/sdk) za detalje.
