...
Sealcode Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

:::note
Sealcode Go je trenutno u beta fazi.
:::

Go radi kao bilo koji drugi provajder u Sealcode-u. Pretplatite se na Sealcode Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Sealcode.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Sealcode-om.
:::
...
Sealcode Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

Sealcode Go radi kao bilo koji drugi provajder u Sealcode-u.

1. Prijavite se na **<a href={console}>Sealcode Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `Sealcode Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Sealcode Go.
:::
...
Sealcode Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Sealcode konfiguraciji
koristi format `sealcode-go/<model-id>`. Na primjer, za Kimi K2.6, koristili biste
`sealcode-go/kimi-k2.6` u svojoj konfiguraciji.
...
Napravili smo Sealcode Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Sealcode koristite i bilo kojeg drugog provajdera.
