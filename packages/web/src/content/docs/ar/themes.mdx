...
مع Sealcode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Sealcode سمتنا `sealcode`.
...
يأتي Sealcode مع عدة سمات مدمجة.
...
- يريدون أن يطابق Sealcode مظهر terminal لديهم
...
يدعم Sealcode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/sealcode/themes/*.json` أو `$XDG_CONFIG_HOME/sealcode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.sealcode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.sealcode/themes/*.json`
