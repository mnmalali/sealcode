---
title: المزوّدون
description: استخدام أي موفّر LLM في Sealcode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Sealcode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Sealcode.
...
في `~/.local/share/sealcode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Sealcode.
...
Sealcode Zen هي قائمة نماذج يوفّرها فريق Sealcode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Sealcode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Sealcode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر sealcode، ثم انتقل إلى [github.com/mnmalali/sealcode/auth](https://github.com/mnmalali/sealcode/auth).
...
يعمل مثل أي مزوّد آخر في Sealcode واستخدامه اختياري بالكامل.

---

## Sealcode Go

Sealcode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Sealcode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Sealcode.

1. شغّل الأمر `/connect` في TUI، واختر `Sealcode Go`، ثم انتقل إلى [github.com/mnmalali/sealcode/auth](https://github.com/mnmalali/sealcode/zen).
...
لاستخدام Amazon Bedrock مع Sealcode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل sealcode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `sealcode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Sealcode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين sealcode لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل sealcode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات sealcode.
...
تخزين الرمز في مساحة تخزين مصادقة sealcode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Sealcode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Sealcode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`sealcode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع sealcode:
...
لاستخدام Google Vertex AI مع Sealcode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر sealcode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [sealcode-helicone-session](https://github.com/H2Shami/sealcode-helicone-session) لتسجيل كل محادثة Sealcode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Sealcode مدرجة كجلسة مستقلة.
...
يمكنك ضبط sealcode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط sealcode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط sealcode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Sealcode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/sealcode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Sealcode:
...
3. انسخ مفتاح API لاستخدامه في Sealcode.
...
6. **مهم**: قبل استخدام نماذج السحابة في Sealcode، يجب سحب معلومات النموذج محليا:
...
Sealcode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Sealcode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Sealcode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Sealcode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Sealcode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات sealcode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات sealcode. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Sealcode:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات sealcode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع sealcode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `sealcode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Sealcode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `sealcode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات sealcode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات sealcode.
