...
Sealcode Go هو اشتراك منخفض التكلفة — **$5 للشهر الأول**، ثم **$10/شهريًا** — يمنحك وصولًا موثوقًا إلى نماذج البرمجة المفتوحة الشائعة.

:::note
Sealcode Go حاليًا في المرحلة التجريبية.
:::

يعمل Go مثل أي مزود آخر في Sealcode. تشترك في Sealcode Go وتحصل على مفتاح API الخاص بك. وهو **اختياري تمامًا**، ولا تحتاج إلى استخدامه لاستخدام Sealcode.
...
:::tip
اختبرنا مجموعة مختارة من النماذج والمزودين الذين يعملون جيدًا مع Sealcode.
:::
...
يمنحك Sealcode Go الوصول إلى هذه النماذج مقابل **$5 للشهر الأول**، ثم **$10/شهريًا**.

---

## كيف يعمل

يعمل Sealcode Go مثل أي مزود آخر في Sealcode.

1. تسجّل الدخول إلى **<a href={console}>Sealcode Zen</a>**، وتشترك في Go، ثم تنسخ مفتاح API الخاص بك.
2. تشغّل الأمر `/connect` في TUI، وتختار `Sealcode Go`، ثم تلصق مفتاح API الخاص بك.
3. شغّل `/models` في TUI لرؤية قائمة النماذج المتاحة عبر Go.

:::note
يمكن لعضو واحد فقط في كل workspace الاشتراك في Sealcode Go.
:::
...
يتضمن Sealcode Go الحدود التالية:
...
يستخدم [model id](/docs/config/#models) في إعدادات Sealcode لديك التنسيق `sealcode-go/<model-id>`. على سبيل المثال، بالنسبة إلى Kimi K2.6، ستستخدم `sealcode-go/kimi-k2.6` في إعداداتك.
...
4. **عدم فرض قيود ارتباط** من خلال السماح لك أيضًا باستخدام أي مزود آخر مع Sealcode.
