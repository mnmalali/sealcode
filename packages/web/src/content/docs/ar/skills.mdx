...
تتيح مهارات الوكيل لـ Sealcode اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Sealcode في هذه المواقع:

- إعدادات المشروع: `.sealcode/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/sealcode/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Sealcode صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.sealcode/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/sealcode/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.sealcode/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Sealcode المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `sealcode.json`:
...
**للوكلاء المدمجين** (في `sealcode.json`):
