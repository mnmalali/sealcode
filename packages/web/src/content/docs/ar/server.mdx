---
title: الخادم
description: تفاعل مع خادم sealcode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `sealcode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل sealcode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `sealcode.local` |
...
عيّن `SEALCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `sealcode`، أو عيّن `SEALCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `sealcode serve` و `sealcode web`.
...
عند تشغيل `sealcode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم sealcode للتفاعل مع sealcode برمجيا.
:::

تتيح هذه البنية لـ sealcode دعم عدة عملاء وتمكّنك من التفاعل مع sealcode برمجيا.

يمكنك تشغيل `sealcode serve` لبدء خادم مستقل. إذا كانت واجهة sealcode في terminal (TUI)
قيد التشغيل، فسيبدأ `sealcode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Sealcode لـ [IDE](/docs/ide).
...
يعرض خادم sealcode واجهات API التالية.
