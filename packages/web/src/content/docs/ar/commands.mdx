...
أنشئ الملف `.sealcode/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Sealcode أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Sealcode:
...
- على مستوى النظام: `~/.config/sealcode/commands/`
- لكل مشروع: `.sealcode/commands/`
...
يتضمن sealcode عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
