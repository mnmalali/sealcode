...
_Model Bağlam Protokolü_ veya MCP kullanarak sealcode'a harici araçlar ekleyebilirsiniz. sealcode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [sealcode Config](https://github.com/mnmalali/sealcode/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/sealcode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
sealcode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde sealcode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa sealcode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `sealcode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından sealcode, belirteçleri `~/.local/share/sealcode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra sealcode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi sealcode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve sealcode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
