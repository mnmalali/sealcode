---
title: Sağlayıcılar
description: sealcode'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

sealcode, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı sealcode yapılandırmanızda yapılandırın.
...
`~/.local/share/sealcode/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları sealcode'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Sealcode Zen, sealcode ekibi tarafından sağlanan modellerin bir listesidir.
sealcode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Sealcode Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, sealcode'u seçin ve [github.com/mnmalali/sealcode/auth](https://github.com/mnmalali/sealcode/auth)'ye gidin.
...
sealcode'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Sealcode Go

Sealcode Go, sealcode ile iyi çalıştığı test edilmiş ve doğrulanmış, sealcode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Sealcode Go`'yu seçin ve [github.com/mnmalali/sealcode/auth](https://github.com/mnmalali/sealcode/zen) adresine gidin.
...
Amazon Bedrock'u sealcode ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   sealcode'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `sealcode.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi sealcode'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
sealcode'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   sealcode'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Sealcode yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
sealcode, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. sealcode'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`sealcode.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`sealcode.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi sealcode'la kullanmak için:
...
Google Vertex AI'yi sealcode ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları sealcode çalıştırılırken ayarlayın.
...
Helicone'un sealcode aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her sealcode görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [sealcode-helicone-session](https://github.com/H2Shami/sealcode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her sealcode görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
sealcode'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla sealcode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla sealcode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini sealcode için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/sealcode)'a bakın.
:::
...
Ollama Cloud'u sealcode ile kullanmak için:
...
3. sealcode'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: sealcode'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Sealcode Zen, sealcode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Sealcode Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Sealcode Zen**'i arayın.

   ```txt
   /connect
   ```

3. sealcode API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   sealcode yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca sealcode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Sealcode ile kullanmak için:
...
Modelleri sealcode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı sealcode ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `sealcode.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları sealcode'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `sealcode auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için sealcode yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, sealcode yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
