---
title: Sunucu
description: sealcode sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`sealcode serve` komutu, sealcode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `sealcode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `SEALCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `sealcode` değeridir; değiştirmek isterseniz `SEALCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `sealcode serve` hem de `sealcode web` için geçerlidir.
...
`sealcode` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
sealcode ile programatik etkileşim için sealcode sunucusunu kullanın.
:::

Bu mimari, sealcode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`sealcode serve` ile bağımsız bir sunucu başlatabilirsiniz. sealcode TUI açıksa, `sealcode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum sealcode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
sealcode sunucusu aşağıdaki API'leri sunar.
