...
description: Sealcode, LSP sunucularınızla bütünleşir.
---

Sealcode, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

Sealcode, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`SEALCODE_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
sealcode bir dosyayı açtığında:
...
LSP sunucularını, sealcode yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
