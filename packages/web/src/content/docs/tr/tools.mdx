...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. sealcode yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
sealcode içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `SEALCODE_EXPERIMENTAL_LSP_TOOL=true` (veya `SEALCODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca sealcode provider kullanılırken veya `SEALCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

sealcode başlatırken etkinleştirmek için:
