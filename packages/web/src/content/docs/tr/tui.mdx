---
title: TUI
description: Sealcode terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Sealcode, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

Sealcode'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
Sealcode TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
Sealcode'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
Sealcode'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `sealcode.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `SEALCODE_TUI_CONFIG` kullanın.
