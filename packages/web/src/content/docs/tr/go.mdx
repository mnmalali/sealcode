...
Sealcode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
Sealcode Go şu anda beta olarak mevcut.
:::

Go, Sealcode'daki diğer sağlayıcılar gibi çalışır. Sealcode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Sealcode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Sealcode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Sealcode Go, Sealcode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Sealcode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Sealcode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Sealcode Go'ya abone olabilir.
:::
...
Sealcode Go aşağıdaki limitleri içerir:
...
Sealcode yapılandırmanızdaki [model id](/docs/config/#models) formatı `sealcode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `sealcode-go/kimi-k2.6` kullanmalısınız.
...
Sealcode Go'yu şu amaçlarla oluşturduk:
...
4. Sealcode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
