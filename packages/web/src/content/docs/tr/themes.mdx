...
sealcode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak sealcode kendi `sealcode` temasını kullanır.
...
sealcode birden fazla yerleşik temayla gelir.
...
- sealcode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
sealcode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/sealcode/themes/*.json` veya `$XDG_CONFIG_HOME/sealcode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.sealcode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.sealcode/themes/*.json`
