---
title: Eklentiler
description: Sealcode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Sealcode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Sealcode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.sealcode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/sealcode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/sealcode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/sealcode/sealcode.json`)
2. Project config (`sealcode.json`)
3. Global eklenti dizini (`~/.config/sealcode/plugins/`)
4. Proje eklenti dizini (`.sealcode/plugins/`)
...
Sealcode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Sealcode SDK istemcisi.
...
Sealcode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Sealcode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Sealcode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Sealcode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Sealcode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Sealcode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://github.com/mnmalali/sealcode/docs/sdk) bakın.
