---
title: サーバー
description: HTTP 経由で Sealcode サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`sealcode serve` コマンドは、Sealcode クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `sealcode.local` |
...
HTTP 基本認証でサーバーを保護するには、`SEALCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `sealcode` になるか、`SEALCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`sealcode serve` と `sealcode web` の両方に当てはまります。
...
`sealcode` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Sealcode サーバーを使用して、プログラムで Sealcode と対話します。
:::

`sealcode serve` を実行してスタンドアロンサーバーを起動できます。すでに Sealcode TUI を実行している場合でも、`sealcode serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Sealcode [IDE](/docs/ide) プラグインによって使用されます。
