---
title: TUI
description: Sealcode ターミナルユーザーインターフェイスの使用。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Sealcode は、LLM を使用してプロジェクトを作業するための対話型ターミナルインターフェイスまたは TUI を提供します。

Sealcode を実行すると、現在のディレクトリの TUI が開始されます。
...
Sealcode TUI を使用する場合、「`/`」に続いてコマンド名を入力すると、アクションをすばやく実行できます。例えば：
...
Sealcode にプロバイダーを追加します。利用可能なプロバイダーから選択し、その API キーを追加できます。
...
Sealcode を終了します。 _エイリアス_: `/quit`、`/q`
...
これは、サーバー/ランタイムの動作を構成する `sealcode.json` とは別です。
...
カスタム TUI 設定パスをロードするには、`SEALCODE_TUI_CONFIG` を使用します。
