---
title: LSP-серверы
description: sealcode интегрируется с вашими серверами LSP.
---

sealcode интегрируется с вашим протоколом языкового сервера (LSP), чтобы помочь LLM взаимодействовать с вашей кодовой базой. Он использует диагностику для предоставления обратной связи LLM.

---

## Встроенные

sealcode поставляется с несколькими встроенными LSP-серверами для популярных языков:
...
:::note
Вы можете отключить автоматическую загрузку LSP-сервера, установив для переменной среды `SEALCODE_DISABLE_LSP_DOWNLOAD` значение `true`.
:::
...
Когда sealcode открывает файл, он:
...
Вы можете настроить серверы LSP через раздел `lsp` в конфигурации sealcode.
