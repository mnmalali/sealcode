...
Создайте `.sealcode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию sealcode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем sealcode [config](/docs/config):
...
- Глобальный: `~/.config/sealcode/commands/`
- Для каждого проекта: `.sealcode/commands/`
...
sealcode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
