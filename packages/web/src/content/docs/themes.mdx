...
With Sealcode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Sealcode uses our own `sealcode` theme.
...
Sealcode comes with several built-in themes.
...
- Want Sealcode to match their terminal's appearance
...
Sealcode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/sealcode/themes/*.json` or `$XDG_CONFIG_HOME/sealcode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.sealcode/themes/*.json`
4. **Current working directory** - Defined in `./.sealcode/themes/*.json`
