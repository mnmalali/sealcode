---
title: Providers
description: Using any LLM provider in Sealcode.
---

import config from "../../../config.mjs"
export const console = config.console

Sealcode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Sealcode config.
...
in `~/.local/share/sealcode/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Sealcode
config.
...
Sealcode Zen is a list of models provided by the Sealcode team that have been
tested and verified to work well with Sealcode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Sealcode Zen.
:::

1. Run the `/connect` command in the TUI, select `Sealcode Zen`, and head to [github.com/mnmalali/sealcode/auth](https://github.com/mnmalali/sealcode/zen).
...
It works like any other provider in Sealcode and is completely optional to use.

---

## Sealcode Go

Sealcode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Sealcode team that have been
tested and verified to work well with Sealcode.

1. Run the `/connect` command in the TUI, select `Sealcode Go`, and head to [github.com/mnmalali/sealcode/auth](https://github.com/mnmalali/sealcode/zen).
...
To use Amazon Bedrock with Sealcode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running sealcode:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY sealcode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile sealcode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX sealcode
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `sealcode.json`:

   ```json title="sealcode.json"
   {
     "$schema": "https://github.com/mnmalali/sealcode/config.json",
     "provider": {
       "amazon-bedrock": {
...
Sealcode. Anthropic explicitly prohibits this.

Previous versions of Sealcode came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Sealcode with zero setup:
...
You can configure sealcode to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="sealcode.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://github.com/mnmalali/sealcode/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for sealcode to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX sealcode
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX sealcode
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your sealcode config.
...
Sealcode supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Sealcode auto-creates a Model Access Key and discovers your available Models & Inference Routers.
...
3. Your browser opens to authorize Sealcode. Sign in and approve.

   :::note
   Sealcode creates a Model Access Key named `sealcode-oauth-<timestamp>` in your DigitalOcean account. You can rotate or revoke it from the **Model Access Keys** page in the "Manage" section of the DigitalOcean console under Inference.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Sealcode surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Sealcode forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Sealcode is experimental. Features, configuration, and
behavior may change in future releases.
:::

Sealcode integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in sealcode auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Sealcode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Sealcode
to only use your own GitLab-hosted instance, add the following to your
`sealcode.json` file. It is also recommended to disable session sharing.
...
Customize through `sealcode.json`:
...
When a `duo-workflow-*` model is selected, Sealcode will:
...
4. Route tool execution requests through Sealcode's permission-gated tool system
...
To use your GitHub Copilot subscription with sealcode:
...
To use Google Vertex AI with Sealcode:
...
2. Set the required environment variables:
   ...
   Set them while running sealcode.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id sealcode
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through sealcode, you can always configure it yourself.
...
```jsonc title="~/.config/sealcode/sealcode.jsonc"
{
  "$schema": "https://github.com/mnmalali/sealcode/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [sealcode-helicone-session](https://github.com/H2Shami/sealcode-helicone-session) plugin to automatically log each Sealcode conversation as a session in Helicone.

```bash
npm install -g sealcode-helicone-session
```
...
```json title="sealcode.json"
{
  "plugin": ["sealcode-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Sealcode conversation listed as a separate session.
...
You can configure sealcode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="sealcode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://github.com/mnmalali/sealcode/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure sealcode to use local models through LM Studio.

```json title="sealcode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://github.com/mnmalali/sealcode/config.json",
  "provider": {
    "lmstudio": {
...
You can configure sealcode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Sealcode. See the [Ollama integration docs](https://docs.ollama.com/integrations/sealcode) for details.
:::

```json title="sealcode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://github.com/mnmalali/sealcode/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Sealcode:
...
3. Copy the API key for use in Sealcode.
...
6. **Important**: Before using cloud models in Sealcode, you must pull the model information locally:
...
Sealcode Zen is a list of tested and verified models provided by the Sealcode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Sealcode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Sealcode Zen**.

   ```txt
   /connect
   ```

3. Enter your Sealcode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your sealcode config.
...
5. You can also customize them through your sealcode config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' sealcode
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group sealcode
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Sealcode:
...
You can also customize models through your sealcode config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with sealcode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in sealcode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `sealcode.json` file in your project directory:

   ```json title="sealcode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://github.com/mnmalali/sealcode/config.json",
     "provider": {
       "myprovider": {
...
```json title="sealcode.json" {9,11,17-20}
{
  "$schema": "https://github.com/mnmalali/sealcode/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Sealcode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `sealcode auth list` to see if the credentials
...
2. For custom providers, check the sealcode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your sealcode config.
