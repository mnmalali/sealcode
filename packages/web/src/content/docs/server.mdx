---
title: Server
description: Interact with sealcode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `sealcode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an sealcode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `sealcode.local` |
...
Set `SEALCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `sealcode`, or set `SEALCODE_SERVER_USERNAME` to override it. This applies to both `sealcode serve` and `sealcode web`.
...
When you run `sealcode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the sealcode server to interact with sealcode programmatically.
:::

This architecture lets sealcode support multiple clients and allows you to interact with sealcode programmatically.

You can run `sealcode serve` to start a standalone server. If you have the
sealcode TUI running, `sealcode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Sealcode [IDE](/docs/ide) plugins.
...
The sealcode server exposes the following APIs.
