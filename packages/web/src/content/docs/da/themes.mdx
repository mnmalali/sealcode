...
Med Sealcode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Sealcode vårt eget `sealcode`-tema.
...
Sealcode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/sealcode/themes/*.json` eller `$XDG_CONFIG_HOME/sealcode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.sealcode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.sealcode/themes/*.json`
