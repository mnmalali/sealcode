---
title: Plugins
description: Skriv dine egne plugins for at udvide Sealcode.
---

Plugins giver dig mulighed for at udvide Sealcode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Sealcodes standardadfærd.
...
- `.sealcode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/sealcode/node_modules/`.
...
1. Global konfiguration (`~/.config/sealcode/sealcode.json`)
2. Projektkonfiguration (`sealcode.json`)
3. Global plugin-mappe (`~/.config/sealcode/plugins/`)
4. Projekt plugin bibliotek (`.sealcode/plugins/`)
...
Sealcode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En sealcode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide sealcode.
...
:::note
Hvis du bruger Sealcode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå sealcode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til sealcode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som sealcode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for sealcode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://github.com/mnmalali/sealcode/docs/sdk) for detaljer.
