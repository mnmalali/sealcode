---
title: Server
description: Interager med sealcode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `sealcode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en sealcode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `sealcode.local` |
...
Indstil `SEALCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `sealcode`, eller indstil `SEALCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `sealcode serve` og `sealcode web`.
...
Når du kører `sealcode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug sealcode-serveren til at interagere med sealcode programmatisk.
:::

Denne arkitektur lader sealcode understøtte flere klienter og lader dig interagere med sealcode programmatisk.

Du kan køre `sealcode serve` for at starte en selvstændig server. Hvis du har
sealcode TUI kørende, vil `sealcode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Sealcode [IDE](/docs/ide) plugins.
...
Sealcode-serveren viser følgende API'er.
