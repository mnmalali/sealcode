---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Sealcode.
---

import config from "../../../../config.mjs"
export const console = config.console

Sealcode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Sealcode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/sealcode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Sealcode-konfigurationen.
...
Sealcode Zen er en liste over modeller leveret af Sealcode-teamet, som er blevet testet og verificeret til at fungere godt med Sealcode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Sealcode Zen.
:::
...
Det fungerer som alle andre udbydere i Sealcode og er helt valgfrit at bruge.

---

## Sealcode Go

Sealcode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Sealcode-teamet, som er testet og verificeret til at fungere godt med Sealcode.

1. Kør kommandoen `/connect` i TUI, vælg `Sealcode Go`, og gå til [github.com/mnmalali/sealcode/auth](https://github.com/mnmalali/sealcode/zen).
...
Sådan bruger du Amazon Bedrock med Sealcode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører sealcode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `sealcode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Sealcode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere sealcode til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at sealcode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem sealcode-konfigurationen.
...
gemme tokenet i sealcode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Sealcode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Sealcode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`sealcode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med Sealcode:
...
Sådan bruger du Google Vertex AI med Sealcode:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører sealcode.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem sealcode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [sealcode-helicone-session](https://github.com/H2Shami/sealcode-helicone-session) plugin for automatisk at logge hver Sealcode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Sealcode-samtale opført som en separat session.
...
Du kan konfigurere sealcode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere sealcode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere sealcode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Sealcode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/sealcode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Sealcode:
...
3. Kopier API-nøglen til brug i Sealcode.
...
6. **Vigtigt**: Før du bruger skymodeller i Sealcode, skal du hente modelinformationen lokalt:
...
Sealcode Zen er en liste over testede og verificerede modeller leveret af Sealcode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Sealcode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Sealcode Zen**.

   ```txt
   /connect
   ```

3. Indtast Sealcode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem sealcode-konfigurationen.
...
5. Du kan også tilpasse dem gennem sealcode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Sealcode:
...
Du kan også tilpasse modeller gennem sealcode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med sealcode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `sealcode.json`-filen i projektkataloget:
...
`limit`-felterne lader Sealcode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `sealcode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek sealcode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i sealcode-konfigurationen.
