---
title: Custom Tools
description: Create tools the LLM can call in sealcode.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside sealcode's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.sealcode/tools/` directory of your project.
- Or globally, by placing them in `~/.config/sealcode/tools/`.
