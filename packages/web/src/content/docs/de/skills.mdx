...
Agent Skills erlauben Sealcode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Sealcode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Sealcode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.sealcode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/sealcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.sealcode/skills/git-release/SKILL.md` zum Beispiel so:
...
Sealcode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `sealcode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `sealcode.json`):
