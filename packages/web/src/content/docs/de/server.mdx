---
title: Server
description: Nutze den sealcode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `sealcode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein sealcode-Client nutzen kann.
...
Setze `SEALCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `sealcode`, kann aber mit `SEALCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `sealcode serve` und `sealcode web`.
...
Wenn du `sealcode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den sealcode-Server, um sealcode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `sealcode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `sealcode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Sealcode-[IDE](/docs/ide)-Plugins.
...
Der sealcode-Server stellt folgende APIs bereit.
