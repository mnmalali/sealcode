...
Sealcode Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

:::note
Sealcode Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Provider in Sealcode. Du abonnierst Sealcode Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
Sealcode zu verwenden.
...
Sealcode Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Sealcode Go funktioniert wie jeder andere Provider in Sealcode.

1. Du meldest dich bei **<a href={console}>Sealcode Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Sealcode Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Sealcode Go abonnieren.
:::
...
Sealcode Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Sealcode Config verwendet das Format `sealcode-go/<model-id>`. Für Kimi K2.6 würdest du beispielsweise `sealcode-go/kimi-k2.6` in deiner Config verwenden.
...
Wir haben Sealcode Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Sealcode zu nutzen.
