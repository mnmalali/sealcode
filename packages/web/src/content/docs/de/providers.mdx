---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Sealcode.
---

import config from "../../../../config.mjs"
export const console = config.console

Sealcode verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Sealcode-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Sealcode anpassen
config.
...
Sealcode Zen ist eine Liste von Modellen, die vom Sealcode-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Sealcode funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Sealcode Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Sealcode aus und gehen Sie zu [github.com/mnmalali/sealcode/auth](https://github.com/mnmalali/sealcode/auth).
...
Es funktioniert wie jeder andere Anbieter in Sealcode und ist völlig optional.

---

## Sealcode Go

Sealcode Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Sealcode-Team getestet und verifiziert wurden, dass sie gut mit Sealcode funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Sealcode Go` und gehen Sie zu [github.com/mnmalali/sealcode/auth](https://github.com/mnmalali/sealcode/zen).
...
So verwenden Sie Amazon Bedrock mit Sealcode:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Sealcode ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `sealcode.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Sealcode wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können sealcode so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Sealcode ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Sealcode-Konfiguration hinzufügen.
...
um das Token im Sealcode-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Sealcode verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Sealcode
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`sealcode.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Sealcode:
...
So verwenden Sie Google Vertex AI mit Sealcode:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Sealcode ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Sealcode konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [sealcode-helicone-session](https://github.com/H2Shami/sealcode-helicone-session)-Plugin, um jede Sealcode-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Sealcode-Konversation als separate Sitzung aufgeführt.
...
Sie können Sealcode für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Sealcode über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Sealcode über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Sealcode konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/sealcode).
:::
...
So verwenden Sie Ollama Cloud mit Sealcode:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Sealcode.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Sealcode verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Sealcode Zen ist eine Liste getesteter und verifizierter Modelle, die vom Sealcode-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Sealcode Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Sealcode Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Sealcode API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Sealcode-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Sealcode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Sealcode:
...
Sie können Modelle auch über Ihre Sealcode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Sealcode verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `sealcode.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Sealcode erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `sealcode auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Sealcode-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Sealcode-Konfiguration übereinstimmt.
