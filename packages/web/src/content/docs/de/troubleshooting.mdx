...
Wenn Sealcode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\sealcode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `sealcode --log-level DEBUG`.

---

## Speicher

sealcode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/sealcode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\sealcode`
...
Sealcode Desktop startet im Hintergrund einen lokalen Sealcode-Server (`sealcode-cli`-Sidecar).
...
- Nur macOS: `Sealcode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/sealcode/sealcode.jsonc` (oder `~/.config/sealcode/sealcode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/sealcode/sealcode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\sealcode\sealcode.jsonc`
...
Sealcode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/sealcode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\sealcode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Sealcode ihn neu aufbauen kann.

1. Beende Sealcode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/sealcode`
- **Linux**: loesche `~/.cache/sealcode` (oder fuehre aus: `rm -rf ~/.cache/sealcode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\sealcode`

3. Starte Sealcode Desktop neu.

---

### Server-Verbindungsprobleme beheben

Sealcode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `sealcode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `SEALCODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `SEALCODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Sealcode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Sealcode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Sealcode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Sealcode in den OS-Einstellungen aktiviert sind, und
...
1. Beende Sealcode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Sealcode Desktop):

- `sealcode.settings.dat` (desktop default server URL)
- `sealcode.global.dat` und `sealcode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Sealcode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `sealcode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\sealcode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. sealcode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\sealcode`

2. Starte sealcode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt sealcode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
sealcode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
