---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Sealcode zu erweitern.
---

Mit Plugins können Sie Sealcode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Sealcode zu ändern.
...
- `.sealcode/plugins/` – Plugins auf Projektebene
- `~/.config/sealcode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/sealcode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/sealcode/sealcode.json`)
2. Projektkonfiguration (`sealcode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/sealcode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.sealcode/plugins/`)
...
Sealcode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Sealcode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Sealcode erweitern können.
...
:::note
Wenn Sie die Sealcode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Sealcode `.env`-Dateien liest:
...
Plugins können Sealcode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Sealcode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Sealcode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://github.com/mnmalali/sealcode/docs/sdk).
