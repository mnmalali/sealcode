---
title: Regeln
description: Lege eigene Anweisungen fuer sealcode fest.
---

Du kannst sealcode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in sealcode den Befehl `/init` aus.
...
So kann sealcode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht sealcode sie zu erweitern.
...
sealcode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/sealcode/AGENTS.md` speichern.
Diese gelten in allen sealcode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Sealcode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/sealcode/AGENTS.md` existiert)
...
Beim Start sucht sealcode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/sealcode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `sealcode.json` oder in `~/.config/sealcode/sealcode.json` angeben.
...
sealcode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit sealcode.json

Empfohlen ist das `instructions`-Feld in der `sealcode.json`:
...
Du kannst sealcode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `sealcode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
