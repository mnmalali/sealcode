...
Mit Sealcode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Sealcode das `sealcode`-Theme.
...
Sealcode liefert mehrere eingebaute Themes mit.
...
- Sealcode optisch ans Terminal anpassen wollen
...
Sealcode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/sealcode/themes/*.json` oder `$XDG_CONFIG_HOME/sealcode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.sealcode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.sealcode/themes/*.json`
