...
Sie können externe Tools zu Sealcode hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Sealcode unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Sealcode Config](https://github.com/mnmalali/sealcode/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/sealcode`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Sealcode übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Sealcode Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Sealcode beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `sealcode mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Sealcode die Token sicher in `~/.local/share/sealcode/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Sealcode verfügbar. Sie können sie also wie jedes andere Tool über die Sealcode-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Sealcode mit Ihrem Sentry-Konto zu verbinden.
