...
Sealcode Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

:::note
Sealcode Go is currently in beta.
:::

Go works like any other provider in Sealcode. You subscribe to Sealcode Go and
...
Sealcode Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

Sealcode Go works like any other provider in Sealcode.

1. You sign in to **<a href={console}>Sealcode Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `Sealcode Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to Sealcode Go.
:::
...
Sealcode Go includes the following limits:
...
uses the format `sealcode-go/<model-id>`. For example, for Kimi K2.6, you would
use `sealcode-go/kimi-k2.6` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with Sealcode as well.
