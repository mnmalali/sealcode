...
Tools allow the LLM to perform actions in your codebase. Sealcode comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Sealcode.
...
:::note
This tool is only available when `SEALCODE_EXPERIMENTAL_LSP_TOOL=true` (or `SEALCODE_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the Sealcode provider or when the `SEALCODE_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching Sealcode:
