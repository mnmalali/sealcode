---
title: Complementos
description: Escriba sus propios complementos para extender Sealcode.
---

Los complementos le permiten extender Sealcode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Sealcode.
...
- `.sealcode/plugins/` - Complementos a nivel de proyecto
- `~/.config/sealcode/plugins/` - Complementos globales
...
```json title="sealcode.json"
{
  ...
  "plugin": ["sealcode-helicone-session", "sealcode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/sealcode/node_modules/`.
...
1. Configuración global (`~/.config/sealcode/sealcode.json`)
2. Configuración del proyecto (`sealcode.json`)
3. Directorio global de complementos (`~/.config/sealcode/plugins/`)
4. Directorio de complementos del proyecto (`.sealcode/plugins/`)
...
```json title=".sealcode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Sealcode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".sealcode/plugins/my-plugin.ts"
...
```js title=".sealcode/plugins/example.js"
...
- `client`: Un cliente SDK sealcode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar sealcode.
...
```js title=".sealcode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "sealcode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Sealcode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que sealcode lea archivos `.env`:

```javascript title=".sealcode/plugins/env-protection.js"
...
```javascript title=".sealcode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a sealcode:

```ts title=".sealcode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@sealcode-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que sealcode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para sealcode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://github.com/mnmalali/sealcode/docs/sdk) para obtener más detalles.
...
```ts title=".sealcode/plugins/compaction.ts"
import type { Plugin } from "@sealcode-ai/plugin"
...
```ts title=".sealcode/plugins/custom-compaction.ts"
import type { Plugin } from "@sealcode-ai/plugin"
