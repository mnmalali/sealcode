...
description: Cree herramientas que LLM pueda llamar en sealcode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de sealcode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.sealcode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/sealcode/tools/`.
...
```ts title=".sealcode/tools/database.ts" {1}
...
```ts title=".sealcode/tools/math.ts"
import { tool } from "@sealcode-ai/plugin"
...
```ts title=".sealcode/tools/bash.ts"
import { tool } from "@sealcode-ai/plugin"
...
```ts title=".sealcode/tools/project.ts" {8}
import { tool } from "@sealcode-ai/plugin"
...
```python title=".sealcode/tools/add.py"
import sys
...
```ts title=".sealcode/tools/python-add.ts" {10}
import { tool } from "@sealcode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".sealcode/tools/add.py")
