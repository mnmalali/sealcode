...
Con Sealcode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Sealcode usa nuestro propio tema `sealcode`.
...
Sealcode viene con varios temas integrados.
...
- Quiere que Sealcode coincida con la apariencia de su terminal
...
Sealcode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/sealcode/themes/*.json` o `$XDG_CONFIG_HOME/sealcode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.sealcode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.sealcode/themes/*.json`
