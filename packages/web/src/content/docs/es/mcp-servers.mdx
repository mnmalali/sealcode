...
Puede agregar herramientas externas a Sealcode utilizando el _Model Context Protocol_ o MCP. Sealcode admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Sealcode Config](https://github.com/mnmalali/sealcode/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/sealcode`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Sealcode maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Sealcode:
...
Si el servidor requiere autenticación, Sealcode le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `sealcode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Sealcode almacenará los tokens de forma segura en `~/.local/share/sealcode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Sealcode, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Sealcode como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Sealcode a su cuenta Sentry.
