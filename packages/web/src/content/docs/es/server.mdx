---
title: Servidor
description: Interactuar con el servidor sealcode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `sealcode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente sealcode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `sealcode.local` |
...
Configure `SEALCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `sealcode`, o configure `SEALCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `sealcode serve` como a `sealcode web`.
...
Cuando ejecuta `sealcode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor sealcode para interactuar con sealcode mediante programación.
:::

Esta arquitectura permite que sealcode admita múltiples clientes y le permite interactuar con sealcode mediante programación.

Puede ejecutar `sealcode serve` para iniciar un servidor independiente. Si tienes el
sealcode TUI ejecutándose, `sealcode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Sealcode [IDE](/docs/ide).
...
El servidor sealcode expone las siguientes API.
