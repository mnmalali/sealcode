...
Las habilidades del agente permiten a Sealcode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Sealcode busca estas ubicaciones:

- Configuración del proyecto: `.sealcode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/sealcode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Sealcode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.sealcode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/sealcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.sealcode/skills/git-release/SKILL.md` así:
...
Sealcode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `sealcode.json`:
...
**Para agentes integrados** (en `sealcode.json`):
