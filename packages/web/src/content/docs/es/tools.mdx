...
Las herramientas permiten que LLM realice acciones en su código base. Sealcode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Sealcode.
...
:::note
Esta herramienta solo está disponible cuando `SEALCODE_EXPERIMENTAL_LSP_TOOL=true` (o `SEALCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Sealcode o cuando la variable de entorno `SEALCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Sealcode:

```bash
SEALCODE_ENABLE_EXA=1 sealcode
```
