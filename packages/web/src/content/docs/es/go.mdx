...
Sealcode Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

:::note
Sealcode Go está actualmente en beta.
:::

Go funciona como cualquier otro proveedor en Sealcode. Te suscribes a Sealcode Go y
...
Sealcode Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

Sealcode Go funciona como cualquier otro proveedor en Sealcode.

1. Inicias sesión en **<a href={console}>Sealcode Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `Sealcode Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a Sealcode Go.
:::
...
Sealcode Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de Sealcode
usa el formato `sealcode-go/<model-id>`. Por ejemplo, para Kimi K2.6, usarías
`sealcode-go/kimi-k2.6` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con Sealcode.
