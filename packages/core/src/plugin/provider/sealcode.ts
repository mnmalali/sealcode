...
export const SealcodePlugin = PluginV2.define({
  id: PluginV2.ID.make("sealcode"),
  effect: Effect.gen(function* () {
    let hasKey = false
    return {
      "provider.update": Effect.fn(function* (evt) {
        if (evt.provider.id !== ProviderV2.ID.sealcode) return
        hasKey = Boolean(
          process.env.SEALCODE_API_KEY ||
      ...
      "model.update": Effect.fn(function* (evt) {
        if (evt.model.providerID !== ProviderV2.ID.sealcode) return
        if (hasKey) return
