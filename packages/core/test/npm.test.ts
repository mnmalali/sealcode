...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@sealcode/acme")).toBe("@sealcode/acme")
    expect(Npm.sanitize("@sealcode/acme@1.0.0")).toBe("@sealcode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/sealcode/acme.git"
    const expected = win ? "acme@git+https_//github.com/sealcode/acme.git" : spec
