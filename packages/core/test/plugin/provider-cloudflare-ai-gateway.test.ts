...
import { CloudflareAIGatewayPlugin } from "@sealcode-ai/core/plugin/provider/cloudflare-ai-gateway"
      ...
      Effect.gen(function* () {
        ...
        expect(aiGatewayCalls[0]).toEqual({
          ...
          options: {
            ...
            collectLog: false,
            headers: {
              "User-Agent": expect.stringContaining("sealcode/"),
            },
          },
        })
      }),
    ...
    withEnv(cloudflareEnv(), () =>
      Effect.gen(function* () {
        ...
        yield* plugin.trigger(
          "aisdk.sdk",
          {
            ...
            options: {
              name: "cloudflare-ai-gateway",
              headers: {
                "cf-aig-metadata": JSON.stringify({ invoked_by: "header", project: "sealcode" }),
              },
            },
          },
          {},
        ...
        expect(aiGatewayCalls[0]?.options).toMatchObject({
          metadata: { invoked_by: "header", project: "sealcode" },
        })
      }),
    ),
  )
