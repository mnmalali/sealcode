...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("sealcode-find")
  api.delete("sealcode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("sealcode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("sealcode-find", new Highlight(...rest))
    return true
  }
