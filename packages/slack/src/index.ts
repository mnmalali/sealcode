...
import { createSealcode, type ToolPart } from "@sealcode-ai/sdk"
...
console.log("🚀 Starting sealcode server...")
const sealcode = await createSealcode({
  port: 0,
})
console.log("✅ Sealcode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await sealcode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new sealcode session...")
    const { client, server } = sealcode
    ...
    console.log("✅ Created sealcode session:", createResult.data.id)
  ...
  console.log("📝 Sending to sealcode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Sealcode response:", JSON.stringify(result, null, 2))
