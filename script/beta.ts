...
const model = "sealcode/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/sealcode")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with sealcode...`)
  ...
  try {
    await $`sealcode run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  sealcode failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with sealcode")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with sealcode...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/sealcode`.",
