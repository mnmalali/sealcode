...
import { createSealcode } from "@sealcode-ai/sdk"
...
`)
  ...
  const sealcode = await createSealcode({ port: 0 })
  ...
  try {
    ...
    const session = await sealcode.client.session.create()
    const result = await sealcode.client.session
      .prompt({
